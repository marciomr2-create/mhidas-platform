// src/app/api/clubbers/connections/route.ts
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("[clubber connections POST] unexpected error:", error);
    return NextResponse.json(
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    console.error("[clubber connections PATCH] unexpected error:", error);
    return NextResponse.json(
      { ok: false, code: "INTERNAL_ERROR" },
      { status: 500 }
    );
