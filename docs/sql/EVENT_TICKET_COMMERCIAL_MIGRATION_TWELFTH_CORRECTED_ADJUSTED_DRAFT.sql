-- docs/sql/EVENT_TICKET_COMMERCIAL_MIGRATION_TWELFTH_CORRECTED_ADJUSTED_DRAFT.sql
-- Version: v4.8.118-event-ticket-commercial-migration-twelfth-corrected-adjusted-draft-safe
-- Base: v4.8.117-event-ticket-commercial-migration-eleventh-corrected-adjusted-draft-structural-review-safe
--
...
--
-- This draft remains normalized from the schema base. It applies the v4.8.113
...
do $mhidas_guard$
begin
  raise exception 'MHIDAS_PROTECTED_DRAFT_V4_8_118';
end;
$mhidas_guard$;
...
begin
  ...
  for v_item in
    ...
    where schemaname = 'public' and policyname = any(array[
      'commercial_partners_admin_read_v4118',
      'event_ticket_channels_admin_read_v4118',
      'event_ticket_requests_owner_or_admin_read_v4118',
      'event_ticket_signals_admin_read_v4118',
      'event_ticket_audit_admin_read_v4118'
    ])
  loop
  ...
  foreach v_name in array array[
    'event_ticket_credentials_one_active_v4118_uq',
    'event_ticket_retention_policy_active_dimension_v4118_uq',
    'event_ticket_channels_one_active_v4118_uq',
    'event_ticket_receipts_semantic_v4118_uq',
    'event_ticket_signal_transaction_v4118_uq'
  ] loop
  ...
  if cardinality(v_missing) > 0 or cardinality(v_drift) > 0 then
    raise exception 'V4_8_118_BASE_PREFLIGHT_FAILED missing=[%] drift=[%]', array_to_string(v_missing, ','), array_to_string(v_drift, ',');
  end if;
end;
$mhidas_preflight$;
...
create function public.mhidas_ticket_json_matches_contract_v3(
  p_contract text,
  p_value jsonb,
...
begin
  ...
  for v_key,v_child in select key,value from jsonb_each(p_value) loop
    ...
    if v_key='processed_counts' then
      if p_contract<>'retention_checkpoint' or v_type<>'object' or jsonb_object_length(v_child)>7 then return false; end if;
      if exists (
        select 1 from jsonb_each(v_child) x
        where x.key not in ('receipt','audit','signal','click','context','communication','backfill')
      ...
      v_text:=v_child #>> '{}';
      if v_key like '%\_hash' escape '\' or v_key in ('cursor_hash','state_hash') then
...
create function public.mhidas_ticket_verify_commercial_destination_envelope_v1(
  p_ciphertext text,
...
set search_path = public, pg_temp
as $mhidas_plpgsql$
declare v_plaintext text; v_hostname text;
begin
...
create table public.commercial_partners (
  ...
  constraint commercial_partners_key_v4118_check
    check (partner_key ~ '^[a-z0-9][a-z0-9_]{2,79}$'),
  constraint commercial_partners_status_v4118_check
    check (partner_status in ('pending_verification','verified','suspended','deactivated')),
  constraint commercial_partners_verification_v4118_check
    check (
      partner_status <> 'verified'
      or (verified_by_admin_user_id is not null and verified_at is not null)
    ),
  constraint commercial_partners_metadata_v4118_check
    check (public.mhidas_ticket_json_matches_contract_v3('partner_metadata', metadata))
);
...
create table public.commercial_partner_representatives (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint commercial_partner_representatives_identity_v4118_unique
    unique (partner_id, user_id),
  constraint commercial_partner_representatives_role_v4118_check
    check (representative_role in ('owner','commercial','marketing','operations','legal')),
  constraint commercial_partner_representatives_status_v4118_check
    check (representation_status in ('pending','active','suspended','revoked')),
  constraint commercial_partner_representatives_validity_v4118_check
    check (valid_until is null or valid_from is null or valid_until > valid_from),
  constraint commercial_partner_representatives_metadata_v4118_check
    check (public.mhidas_ticket_json_matches_contract_v3('representative_metadata', metadata))
);
...
create table public.event_ticket_trusted_integrations (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_integrations_key_v4118_check
    check (integration_key ~ '^[a-z0-9][a-z0-9_.-]{2,79}$'),
  constraint event_ticket_integrations_namespace_v4118_check
    check (principal_namespace ~ '^[a-z0-9][a-z0-9_.-]{2,79}$'),
  constraint event_ticket_integrations_status_v4118_check
    check (integration_status in ('pending','active','suspended','retired')),
  constraint event_ticket_integrations_partner_v4118_check
  ...
  constraint event_ticket_integrations_metadata_v4118_check
    check (public.mhidas_ticket_json_matches_contract_v3('integration_metadata', metadata))
);
...
create table public.event_ticket_trusted_integration_credential_versions (
  ...
  created_at timestamptz not null default now(),
  constraint event_ticket_credentials_pair_v4118_unique
    unique (integration_id, credential_version_id),
  constraint event_ticket_credentials_version_v4118_unique
    unique (integration_id, credential_version),
  constraint event_ticket_credentials_status_v4118_check
    check (credential_status in ('pending','active','rotating','revoked','expired')),
  constraint event_ticket_credentials_hashes_v4118_check
    check (
  ...
  constraint event_ticket_credentials_validity_v4118_check
    check (valid_until is null or valid_until > valid_from)
);

create unique index event_ticket_credentials_one_active_v4118_uq
...
alter table public.event_ticket_trusted_integrations
  add constraint event_ticket_integrations_current_credential_v4118_fk
...
create constraint trigger event_ticket_integrations_current_credential_v4118_ct
...
create constraint trigger event_ticket_credentials_current_invariant_v4118_ct
...
create table public.event_ticket_trusted_integration_channels (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_integration_channels_identity_v4118_unique
    unique (integration_id, canonical_event_id, channel_scope),
  constraint event_ticket_integration_channels_authority_v4118_unique
    unique (integration_channel_id, integration_id, canonical_event_id),
  constraint event_ticket_integration_channels_authority_scope_v4118_unique
    unique (integration_channel_id, integration_id, canonical_event_id, channel_scope),
  constraint event_ticket_integration_channels_scope_v4118_check
    check (channel_scope in ('purchase_signal','url_validation','conversion_confirmation')),
  constraint event_ticket_integration_channels_status_v4118_check
    check (scope_status in ('pending','active','suspended','revoked')),
  constraint event_ticket_integration_channels_validity_v4118_check
    check (valid_until is null or valid_from is null or valid_until > valid_from)
);
...
create table public.event_ticket_partnership_requests (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_requests_submission_v4118_unique
    unique (partner_id, client_submission_key),
  constraint event_ticket_requests_authorization_v4118_unique
    unique (request_id, canonical_event_id, partner_id),
  constraint event_ticket_requests_type_v4118_check
  ...
  constraint event_ticket_requests_status_v4118_check
    check (request_status in ('pending','needs_info','approved','rejected','withdrawn')),
  constraint event_ticket_requests_hashes_v4118_check
    check (
  ...
  constraint event_ticket_requests_metadata_v4118_check
    check (public.mhidas_ticket_json_matches_contract_v3('request_metadata', metadata))
);
...
create table public.event_ticket_retention_policy_bootstrap_receipts (
  ...
  created_at timestamptz not null default now(),
  constraint event_ticket_retention_bootstrap_status_v4118_check
    check (bootstrap_status = 'completed'),
  constraint event_ticket_retention_bootstrap_count_v4118_check
    check (policy_count = 7),
  constraint event_ticket_retention_bootstrap_hashes_v4118_check
...
create table public.event_ticket_retention_policy_versions (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_retention_policy_version_v4118_unique
    unique (policy_key, policy_version),
  constraint event_ticket_retention_policy_status_v4118_check
    check (policy_status in ('draft','approved','active','retired')),
  constraint event_ticket_retention_policy_dimensions_v4118_check
    check (
      policy_purpose in (
  ...
  constraint event_ticket_retention_policy_action_v4118_check
    check (retention_action in ('delete','anonymize','tombstone')),
  constraint event_ticket_retention_policy_days_v4118_check
    check (retention_days between 1 and 3650),
  constraint event_ticket_retention_policy_hashes_v4118_check
    check (
      legal_basis_reference_hash ~ '^[0-9a-f]{64}$'
      and policy_manifest_hash ~ '^[0-9a-f]{64}$'
      and executor_contract_hash ~ '^[0-9a-f]{64}$'
    ),
  constraint event_ticket_retention_policy_approval_v4118_check
    check (
      policy_status = 'draft'
      or (approved_at is not null and (approved_by_admin_user_id is not null or bootstrap_receipt_id is not null))
    ),
  constraint event_ticket_retention_policy_activation_v4118_check
    check (policy_status <> 'active' or activated_at is not null),
  constraint event_ticket_retention_policy_retirement_v4118_check
    check (
...
alter table public.event_ticket_retention_policy_versions
  add constraint event_ticket_retention_policy_successor_v4118_fk
...
create unique index event_ticket_retention_policy_active_dimension_v4118_uq
...
create table public.event_ticket_retention_minimization_rules (
  ...
  created_at timestamptz not null default now(),
  constraint event_ticket_minimization_rule_identity_v4118_unique
    unique (retention_policy_version_id, evidence_class, source_table, source_field),
  constraint event_ticket_minimization_rule_action_v4118_check
    check (minimization_action in ('delete','nullify','stable_hash','retain','replace_with_tombstone')),
  constraint event_ticket_minimization_rule_hash_v4118_check
    check (
      minimization_action <> 'stable_hash'
      or stable_hash_namespace ~ '^[a-z0-9][a-z0-9._-]{2,80}$'
    ),
  constraint event_ticket_minimization_rule_status_v4118_check
...
create table public.event_ticket_url_validation_authorities (
  ...
  approved_at timestamptz,
  created_at timestamptz not null default now(),
  constraint event_ticket_url_authority_status_v4118_check check (authority_status in ('pending','active','suspended','retired')),
  constraint event_ticket_url_authority_hash_v4118_check check (
    signer_key_hash ~ '^[0-9a-f]{64}$'
    and signer_key_id ~ '^[a-zA-Z0-9][a-zA-Z0-9._:/-]{2,199}$'
    and signature_algorithm in ('ed25519','ecdsa-p256-sha256','rsa-pss-sha256')
  ),
  constraint event_ticket_url_authority_validity_v4118_check check (valid_until is null or valid_until > valid_from)
);
...
create table public.event_ticket_official_source_attestations (
  ...
  created_at timestamptz not null default now(),
  constraint event_ticket_official_source_identity_v4118_unique unique (canonical_event_id, source_url),
  constraint event_ticket_official_source_hashes_v4118_check check (
  ...
  constraint event_ticket_official_source_signature_v4118_check check (
  ...
  constraint event_ticket_official_source_host_v4118_check check (
  ...
  constraint event_ticket_official_source_policy_v4118_check check (commercial_policy_status in ('official_noncommercial','commercial_authorized')),
  constraint event_ticket_official_source_freshness_v4118_check check (expires_at > validated_at and expires_at <= validated_at + interval '30 days')
);
...
create table public.event_ticket_commercial_channels (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_channels_semantic_v4118_unique unique (channel_id, canonical_event_id, integration_id),
  constraint event_ticket_channels_request_v4118_fk foreign key (request_id,canonical_event_id,partner_id)
    references public.event_ticket_partnership_requests(request_id,canonical_event_id,partner_id) on delete restrict,
  constraint event_ticket_channels_scope_v4118_fk foreign key (integration_channel_id,integration_id,canonical_event_id)
  ...
  constraint event_ticket_channels_status_v4118_check check (channel_status in ('draft','authorized','active','paused','expired','revoked')),
  constraint event_ticket_channels_kind_v4118_check check (channel_kind in ('official_reference','affiliate','discount','presale','fixed_media','hybrid')),
  constraint event_ticket_channels_tracking_v4118_check check (tracking_method in ('none','redirect','coupon','postback','webhook','partner_api')),
  constraint event_ticket_channels_financial_v4118_check check (
  ...
  constraint event_ticket_channels_hashes_v4118_check check (
  ...
  constraint event_ticket_channels_active_authority_v4118_check check (
  ...
  constraint event_ticket_channels_metadata_v4118_check check (public.mhidas_ticket_json_matches_contract_v3('channel_metadata', metadata))
);

create unique index event_ticket_channels_one_active_v4118_uq
...
create table public.event_ticket_operation_result_contracts (
  ...
  constraint event_ticket_result_contract_name_v4118_check check (operation_name ~ '^[a-z0-9][a-z0-9._:-]{2,119}$'),
  constraint event_ticket_result_contract_status_v4118_check check (
    result_status ~ '^[a-z0-9][a-z0-9._:-]{2,79}$'
    and result_status not in ('pending','failed')
    and contract_status in ('active','retired')
  ),
  constraint event_ticket_result_contract_version_v4118_check check (
...
create table public.event_ticket_retention_executor_contracts (
  ...
  contract_status text not null default 'active',
  created_at timestamptz not null default now(),
  constraint event_ticket_retention_executor_hashes_v4118_check check (
    canonical_rule_manifest_hash ~ '^[0-9a-f]{64}$' and executor_code_hash ~ '^[0-9a-f]{64}$' and contract_hash ~ '^[0-9a-f]{64}$'
  ),
  constraint event_ticket_retention_executor_status_v4118_check check (contract_status in ('active','retired'))
);
...
create table public.event_ticket_legacy_backfill_runs (
  ...
  created_at timestamptz not null default now(),
  constraint event_ticket_legacy_backfill_run_status_v4118_check check (run_status in ('prepared','validated','cutover_ready','cutover_completed','rolled_back','failed')),
  constraint event_ticket_legacy_backfill_run_hashes_v4118_check check (source_inventory_hash ~ '^[0-9a-f]{64}$' and mapped_checksum ~ '^[0-9a-f]{64}$' and rejection_checksum ~ '^[0-9a-f]{64}$')
);
...
create table public.event_ticket_legacy_backfill_items (
  ...
  constraint event_ticket_legacy_backfill_item_status_v4118_check check (mapping_status in ('mapped','rejected')),
  constraint event_ticket_legacy_backfill_item_hash_v4118_check check (source_row_hash ~ '^[0-9a-f]{64}$')
);
...
create table public.event_ticket_legacy_cutover_checkpoints (
  ...
  approved_at timestamptz,
  created_at timestamptz not null default now(),
  constraint event_ticket_legacy_cutover_status_v4118_check check (checkpoint_status in ('prepared','validated','approved','completed','rolled_back')),
  constraint event_ticket_legacy_cutover_hashes_v4118_check check (dual_read_checksum ~ '^[0-9a-f]{64}$' and rollback_checksum ~ '^[0-9a-f]{64}$')
);
...
create table public.event_ticket_operation_receipts (
  ...
  constraint event_ticket_receipts_credential_pair_v4118_fk
    ...
    on delete restrict,
  constraint event_ticket_receipts_scope_v4118_check
  ...
  constraint event_ticket_receipts_principal_v4118_check
    check (
  ...
  constraint event_ticket_receipts_target_v4118_check
  ...
  constraint event_ticket_receipts_hashes_v4118_check
    check (
  ...
  constraint event_ticket_receipts_state_v4118_check
    check (
...
create unique index event_ticket_receipts_semantic_v4118_uq
...
alter table public.event_ticket_commercial_channels
  add constraint event_ticket_channels_authorization_receipt_v4118_fk
...
create table public.event_ticket_verified_credential_contexts (
  ...
  minimized_at timestamptz,
  constraint event_ticket_context_credential_pair_v4118_fk foreign key (integration_id,credential_version_id)
  ...
  constraint event_ticket_context_semantic_issue_v4118_unique unique (integration_id,credential_version_id,audience,operation_name,request_payload_hash,proof_nonce_hash),
  constraint event_ticket_context_consumed_receipt_v4118_unique unique (consumed_receipt_id),
  constraint event_ticket_context_status_v4118_check check (context_status in ('active','consumed','revoked','expired')),
  constraint event_ticket_context_contract_v4118_check check (audience ~ '^[a-z0-9][a-z0-9._:-]{2,79}$' and operation_name ~ '^[a-z0-9][a-z0-9._:-]{2,119}$'),
  constraint event_ticket_context_hashes_v4118_check check (
  ...
  constraint event_ticket_context_validity_v4118_check check (expires_at > issued_at),
  constraint event_ticket_context_terminal_v4118_check check (
...
create table public.event_ticket_click_attributions (
  ...
  constraint event_ticket_click_channel_semantic_v4118_fk
    ...
    on delete restrict,
  constraint event_ticket_click_semantic_v4118_unique
    unique (click_id, channel_id, canonical_event_id, integration_id),
  constraint event_ticket_click_redirect_v4118_unique unique (redirect_id),
  constraint event_ticket_click_hashes_v4118_check
    check (
...
create table public.event_ticket_purchase_signals (
  ...
  minimized_at timestamptz,
  constraint event_ticket_signal_credential_pair_v4118_fk
    foreign key (integration_id, credential_version_id)
    references public.event_ticket_trusted_integration_credential_versions (
      integration_id,
      credential_version_id
    ...
    on delete restrict,
  constraint event_ticket_signal_channel_semantic_v4118_fk
    foreign key (channel_id, canonical_event_id, integration_id)
    references public.event_ticket_commercial_channels (channel_id, canonical_event_id, integration_id)
    on delete restrict,
  constraint event_ticket_signal_click_semantic_v4118_fk
  ...
  constraint event_ticket_signal_status_v4118_check
    check (signal_status in ('recorded','superseded','tombstoned')),
  constraint event_ticket_signal_hashes_v4118_check
    check (
      ...
      and public.mhidas_ticket_json_matches_contract_v3('signal_evidence', evidence_metadata)
    ),
  constraint event_ticket_signal_trusted_evidence_v4118_check
    check (
  ...
  constraint event_ticket_signal_supersession_target_v4118_unique unique (signal_id,canonical_event_id,integration_id,transaction_family_id),
  constraint event_ticket_signal_supersession_v4118_fk foreign key (supersedes_signal_id,canonical_event_id,integration_id,transaction_family_id)
  ...
  constraint event_ticket_signal_click_requires_channel_v4118_check
    check (click_id is null or channel_id is not null)
);

create unique index event_ticket_signal_transaction_v4118_uq
...
create table public.partner_official_communications (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint partner_communications_status_v4118_check
    check (communication_status in ('draft','submitted','approved','published','paused','revoked','expired')),
  constraint partner_communications_audience_v4118_check
    check (audience_scope in ('all_active_clubbers','event_participants','partner_event_audience')),
  constraint partner_communications_hash_v4118_check
    check (body_hash ~ '^[0-9a-f]{64}$'),
  constraint partner_communications_validity_v4118_check
    check (ends_at is null or starts_at is null or ends_at > starts_at),
  constraint partner_communications_metadata_v4118_check
    check (public.mhidas_ticket_json_matches_contract_v3('communication_metadata', metadata))
);
...
create table public.event_ticket_retention_runs (
  ...
  completed_at timestamptz,
  failed_at timestamptz,
  constraint event_ticket_retention_run_status_v4118_check check (run_status in ('running','completed','failed')),
  constraint event_ticket_retention_run_limit_v4118_check check (batch_limit between 1 and 5000),
  constraint event_ticket_retention_run_counts_v4118_check check (
  ...
  constraint event_ticket_retention_run_hashes_v4118_check check (
    (result_hash is null or result_hash ~ '^[0-9a-f]{64}$') and (error_hash is null or error_hash ~ '^[0-9a-f]{64}$')
    and public.mhidas_ticket_json_matches_contract_v3('retention_checkpoint', checkpoint)
  )
);
...
create table public.event_ticket_commercial_audit_log (
  ...
  minimized_at timestamptz,
  created_at timestamptz not null default now(),
  constraint event_ticket_audit_credential_pair_v4118_fk
    foreign key (integration_id, credential_version_id)
    references public.event_ticket_trusted_integration_credential_versions (
      integration_id,
      credential_version_id
    ...
    on delete restrict,
  constraint event_ticket_audit_identity_v4118_unique
    unique (target_type, target_id, idempotency_key),
  constraint event_ticket_audit_target_v4118_check
    check (target_type in (
  ...
  constraint event_ticket_audit_actor_v4118_check
    check (actor_role in (
      'useclubbers_admin','verified_partner_representative','clubber',
      'automation','system','trusted_ticketing_integration'
    )),
  constraint event_ticket_audit_hashes_v4118_check
    check (
      reason_hash ~ '^[0-9a-f]{64}$'
      and (anonymized_target_hash is null or anonymized_target_hash ~ '^[0-9a-f]{64}$')
      and (anonymized_correlation_hash is null or anonymized_correlation_hash ~ '^[0-9a-f]{64}$')
      and (anonymized_idempotency_hash is null or anonymized_idempotency_hash ~ '^[0-9a-f]{64}$')
      and (before_snapshot is null or public.mhidas_ticket_json_matches_contract_v3('audit_snapshot', before_snapshot))
      and (after_snapshot is null or public.mhidas_ticket_json_matches_contract_v3('audit_snapshot', after_snapshot))
    )
);
...
create table public.event_ticket_backfill_rejections (
  ...
  minimized_at timestamptz,
  created_at timestamptz not null default now(),
  constraint event_ticket_backfill_identity_v4118_unique
    unique (reconciliation_run_key, source_table, source_primary_key),
  constraint event_ticket_backfill_source_v4118_check
    check (source_table in ('partner_ticket_requests','event_groups','event_ticket_intents')),
  constraint event_ticket_backfill_hash_v4118_check
...
insert into public.event_ticket_retention_policy_bootstrap_receipts (
  ...
  '00000000-0000-4118-8000-000000000000'::uuid,
...
values
  ('00000000-0000-4118-8000-000000000001'::uuid,'operation_receipt_br_template',1,'active','operation_receipt','BR','operation_receipt','anonymize',730,repeat('0',64),public.mhidas_ticket_sha256_v1('operation_receipt_br_template_v1'),now(),now(),'00000000-0000-4118-8000-000000000000'::uuid,public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:bootstrap-pending')),
  ('00000000-0000-4118-8000-000000000002'::uuid,'commercial_audit_br_template',1,'active','commercial_audit','BR','commercial_audit','anonymize',1825,repeat('0',64),public.mhidas_ticket_sha256_v1('commercial_audit_br_template_v1'),now(),now(),'00000000-0000-4118-8000-000000000000'::uuid,public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:bootstrap-pending')),
  ('00000000-0000-4118-8000-000000000003'::uuid,'purchase_signal_br_template',1,'active','event_ticket_tracking','BR','purchase_signal','tombstone',730,repeat('0',64),public.mhidas_ticket_sha256_v1('purchase_signal_br_template_v1'),now(),now(),'00000000-0000-4118-8000-000000000000'::uuid,public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:bootstrap-pending')),
  ('00000000-0000-4118-8000-000000000004'::uuid,'click_attribution_br_template',1,'active','event_ticket_tracking','BR','click_attribution','anonymize',365,repeat('0',64),public.mhidas_ticket_sha256_v1('click_attribution_br_template_v1'),now(),now(),'00000000-0000-4118-8000-000000000000'::uuid,public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:bootstrap-pending')),
  ('00000000-0000-4118-8000-000000000005'::uuid,'credential_context_br_template',1,'active','security_audit','BR','credential_context','anonymize',90,repeat('0',64),public.mhidas_ticket_sha256_v1('credential_context_br_template_v1'),now(),now(),'00000000-0000-4118-8000-000000000000'::uuid,public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:bootstrap-pending')),
  ('00000000-0000-4118-8000-000000000006'::uuid,'partner_communication_br_template',1,'active','commercial_governance','BR','partner_communication','tombstone',730,repeat('0',64),public.mhidas_ticket_sha256_v1('partner_communication_br_template_v1'),now(),now(),'00000000-0000-4118-8000-000000000000'::uuid,public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:bootstrap-pending')),
  ('00000000-0000-4118-8000-000000000007'::uuid,'backfill_rejection_br_template',1,'active','commercial_governance','BR','backfill_rejection','anonymize',365,repeat('0',64),public.mhidas_ticket_sha256_v1('backfill_rejection_br_template_v1'),now(),now(),'00000000-0000-4118-8000-000000000000'::uuid,public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:bootstrap-pending'));
...
values
  ('00000000-0000-4118-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','principal_id','nullify',null),
  ('00000000-0000-4118-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','integration_id','nullify',null),
  ('00000000-0000-4118-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','credential_version_id','nullify',null),
  ('00000000-0000-4118-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','principal_namespace','stable_hash','receipt.principal_namespace'),
  ('00000000-0000-4118-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','idempotency_key','stable_hash','receipt.idempotency'),
  ('00000000-0000-4118-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','target_id','stable_hash','receipt.target'),
  ('00000000-0000-4118-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','result_id','stable_hash','receipt.result'),
  ('00000000-0000-4118-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','correlation_id','stable_hash','receipt.correlation'),
  ('00000000-0000-4118-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','target_id','stable_hash','audit.target'),
  ('00000000-0000-4118-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','canonical_event_id','nullify',null),
  ('00000000-0000-4118-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','partner_id','nullify',null),
  ('00000000-0000-4118-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','integration_id','nullify',null),
  ('00000000-0000-4118-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','credential_version_id','nullify',null),
  ('00000000-0000-4118-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','actor_user_id','nullify',null),
  ('00000000-0000-4118-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','correlation_id','stable_hash','audit.correlation'),
  ('00000000-0000-4118-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','idempotency_key','stable_hash','audit.idempotency'),
  ('00000000-0000-4118-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','before_snapshot','nullify',null),
  ('00000000-0000-4118-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','after_snapshot','nullify',null),
  ('00000000-0000-4118-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','user_id','nullify',null),
  ('00000000-0000-4118-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','channel_id','nullify',null),
  ('00000000-0000-4118-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','click_id','nullify',null),
  ('00000000-0000-4118-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','credential_context_id','nullify',null),
  ('00000000-0000-4118-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','external_transaction_hash','stable_hash','signal.transaction'),
  ('00000000-0000-4118-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','evidence_metadata','replace_with_tombstone',null),
  ('00000000-0000-4118-8000-000000000004'::uuid,'click_attribution','event_ticket_click_attributions','user_id','nullify',null),
  ('00000000-0000-4118-8000-000000000004'::uuid,'click_attribution','event_ticket_click_attributions','visitor_hash','stable_hash','click.visitor'),
  ('00000000-0000-4118-8000-000000000004'::uuid,'click_attribution','event_ticket_click_attributions','campaign_hash','nullify',null),
  ('00000000-0000-4118-8000-000000000004'::uuid,'click_attribution','event_ticket_click_attributions','redirect_id','stable_hash','click.redirect'),
  ('00000000-0000-4118-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','context_token_hash','stable_hash','context.token'),
  ('00000000-0000-4118-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','request_payload_hash','stable_hash','context.request'),
  ('00000000-0000-4118-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','proof_nonce_hash','stable_hash','context.nonce'),
  ('00000000-0000-4118-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','proof_signature_hash','stable_hash','context.signature'),
  ('00000000-0000-4118-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','lifecycle_correlation_hash','stable_hash','context.correlation'),
  ('00000000-0000-4118-8000-000000000006'::uuid,'partner_communication','partner_official_communications','canonical_event_id','nullify',null),
  ('00000000-0000-4118-8000-000000000006'::uuid,'partner_communication','partner_official_communications','title','replace_with_tombstone',null),
  ('00000000-0000-4118-8000-000000000006'::uuid,'partner_communication','partner_official_communications','body_hash','stable_hash','communication.body'),
  ('00000000-0000-4118-8000-000000000006'::uuid,'partner_communication','partner_official_communications','approved_by_admin_user_id','nullify',null),
  ('00000000-0000-4118-8000-000000000006'::uuid,'partner_communication','partner_official_communications','published_by_admin_user_id','nullify',null),
  ('00000000-0000-4118-8000-000000000006'::uuid,'partner_communication','partner_official_communications','metadata','replace_with_tombstone',null),
  ('00000000-0000-4118-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','source_primary_key','stable_hash','backfill.source_key'),
  ('00000000-0000-4118-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','rejection_reason','replace_with_tombstone',null),
  ('00000000-0000-4118-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','canonical_event_id','nullify',null),
  ('00000000-0000-4118-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','source_payload_hash','stable_hash','backfill.payload'),
  ('00000000-0000-4118-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','reconciliation_run_key','stable_hash','backfill.run');
...
values
  ('issue_verified_credential_context_v7','issued',1,1),
  ('record_purchase_signal_v12','recorded',1,1),
  ('run_governance_retention_batch_v8','completed',1,1),
  ...
  ('authorize_activate_commercial_channel_v3','active',1,null);
...
insert into public.event_ticket_retention_executor_contracts (
  ...
  ('operation_receipt',10,public.mhidas_ticket_sha256_v1('v4118:operation_receipt:8:fixed'),public.mhidas_ticket_sha256_v1('mhidas.retention.executor.v10:operation_receipt'),public.mhidas_ticket_sha256_v1('v4118:operation_receipt:executor-contract')),
  ('commercial_audit',10,public.mhidas_ticket_sha256_v1('v4118:commercial_audit:10:fixed'),public.mhidas_ticket_sha256_v1('mhidas.retention.executor.v10:commercial_audit'),public.mhidas_ticket_sha256_v1('v4118:commercial_audit:executor-contract')),
  ('purchase_signal',10,public.mhidas_ticket_sha256_v1('v4118:purchase_signal:6:fixed'),public.mhidas_ticket_sha256_v1('mhidas.retention.executor.v10:purchase_signal'),public.mhidas_ticket_sha256_v1('v4118:purchase_signal:executor-contract')),
  ('click_attribution',10,public.mhidas_ticket_sha256_v1('v4118:click_attribution:4:fixed'),public.mhidas_ticket_sha256_v1('mhidas.retention.executor.v10:click_attribution'),public.mhidas_ticket_sha256_v1('v4118:click_attribution:executor-contract')),
  ('credential_context',10,public.mhidas_ticket_sha256_v1('v4118:credential_context:5:fixed'),public.mhidas_ticket_sha256_v1('mhidas.retention.executor.v10:credential_context'),public.mhidas_ticket_sha256_v1('v4118:credential_context:executor-contract')),
  ('partner_communication',10,public.mhidas_ticket_sha256_v1('v4118:partner_communication:5:fixed'),public.mhidas_ticket_sha256_v1('mhidas.retention.executor.v10:partner_communication'),public.mhidas_ticket_sha256_v1('v4118:partner_communication:executor-contract')),
  ('backfill_rejection',10,public.mhidas_ticket_sha256_v1('v4118:backfill_rejection:6:fixed'),public.mhidas_ticket_sha256_v1('mhidas.retention.executor.v10:backfill_rejection'),public.mhidas_ticket_sha256_v1('v4118:backfill_rejection:executor-contract'));
...
create function public.mhidas_ticket_issue_verified_credential_context_v7(
  p_integration_id uuid,
  p_credential_version_id uuid,
...
begin
  ...
  if p_issuer_service_key is null or p_context_token is null or p_nonce is null or p_detached_credential_signature is null
    ...
    raise exception 'ISSUER_SERVICE_KEY_PROOF_DENIED_V6';
  end if;
  if p_expires_at <= now() or p_expires_at > now() + interval '10 minutes' then
    raise exception 'CREDENTIAL_CONTEXT_EXPIRY_OUT_OF_RANGE_V6';
  end if;
  if p_audience !~ '^[a-z0-9][a-z0-9._:-]{2,79}$' or p_operation_name !~ '^[a-z0-9][a-z0-9._:-]{2,119}$' then
    raise exception 'CREDENTIAL_CONTEXT_SCOPE_INVALID_V6';
  end if;
  if p_request_payload_hash !~ '^[0-9a-f]{64}$' or length(p_detached_credential_signature) not between 32 and 4096 then
    raise exception 'CREDENTIAL_CONTEXT_PROOF_INVALID_V6';
  end if;
  ...
  v_canonical_payload := public.mhidas_ticket_canonical_payload_v1(
    'mhidas.credential-context.v6',
    jsonb_build_object(
      'integration_id', p_integration_id,
      'credential_version_id', p_credential_version_id,
      'credential_key_id', v_authority.credential_key_id,
      'signature_algorithm', v_authority.signature_algorithm,
      'verifier_key_hash', v_authority.verifier_key_hash,
      'credential_fingerprint_hash', v_authority.credential_fingerprint_hash,
      'audience', p_audience,
      'operation_name', p_operation_name,
      'context_token_hash', v_token_hash,
      'nonce_hash', v_nonce_hash,
      'request_payload_hash', p_request_payload_hash,
      'expires_at_epoch_ms', floor(extract(epoch from p_expires_at) * 1000)::bigint
    )
  );
  ...
  if public.mhidas_verify_detached_signature_with_key_hash_v1(
    v_authority.credential_key_id,
    v_authority.signature_algorithm,
    v_authority.verifier_key_hash,
    v_authority.credential_fingerprint_hash,
    v_canonical_payload,
    p_detached_credential_signature
  ) is distinct from true then
    raise exception 'CREDENTIAL_KEY_POSSESSION_PROOF_DENIED_V6';
  end if;
  ...
  v_context_id := public.mhidas_ticket_deterministic_uuid_v1(
    public.mhidas_ticket_canonical_payload_v1(
      'mhidas.credential-context-id.v1',
      jsonb_build_object(
        'integration_id', p_integration_id,
        'credential_version_id', p_credential_version_id,
        'audience', p_audience,
  ...
  v_request_hash := p_request_payload_hash;
  v_receipt := public.mhidas_ticket_reserve_operation_receipt_v10(
    ...
    p_integration_id,p_credential_version_id,'issue_verified_credential_context_v7',p_idempotency_key,
  ...
  begin
    ...
    perform public.mhidas_ticket_complete_operation_receipt_v6(
      v_receipt.receipt_id,'issued',1,public.mhidas_ticket_sha256_v1(v_context.context_id::text||':issued')
    );
...
create function public.mhidas_ticket_consume_verified_credential_context_v7(
  p_context_id uuid,
  p_context_token text,
  p_audience text,
  p_operation_name text,
...
declare
  ...
  v_authority record;
begin
  if p_expected_request_payload_hash !~ '^[0-9a-f]{64}$' then
    raise exception 'CREDENTIAL_CONTEXT_REQUEST_HASH_INVALID_V7';
  end if;
  select * into v_authority from public.mhidas_ticket_lock_current_credential_v4(p_integration_id,p_credential_version_id);
  ...
  if not found or v_receipt.receipt_status<>'pending' or v_receipt.operation_scope<>'signal_insert'
    ...
    or v_receipt.principal_type<>'trusted_ticketing_integration'
    or v_receipt.request_hash<>p_expected_request_payload_hash then
    raise exception 'CREDENTIAL_CONTEXT_RECEIPT_SEMANTIC_BINDING_DENIED_V7';
  end if;
  ...
  if not found or v_context.context_status<>'active' or v_context.integration_id<>p_integration_id
    ...
    raise exception 'VERIFIED_CREDENTIAL_CONTEXT_PROOF_NOT_CONSUMABLE_V7';
  end if;
  ...
  if not found then raise exception 'VERIFIED_CREDENTIAL_CONTEXT_CONCURRENT_CONSUMPTION_DENIED_V7'; end if;
  return v_context;
end;
$mhidas_plpgsql$;
...
create function public.mhidas_record_event_ticket_purchase_signal_v12(
...
begin
  ...
  if not public.mhidas_ticket_json_matches_contract_v3('signal_evidence',p_evidence_metadata) then raise exception 'SIGNAL_EVIDENCE_CONTRACT_DENIED_V10'; end if;
  ...
  v_request_hash:=public.mhidas_ticket_sha256_v1(
    public.mhidas_ticket_canonical_payload_v1(
      ...
      jsonb_build_object(
        ...
        'integration_id',p_integration_id,
        'credential_version_id',p_credential_version_id,
        'credential_context_id',p_credential_context_id,
        'context_token_hash',public.mhidas_ticket_sha256_v1(p_context_token),
        'signal_type',p_signal_type,
        'evidence_source',p_evidence_source,
        'external_transaction_hash',p_external_transaction_hash,
        'transaction_family_id',v_transaction_family_id,
        ...
        'retention_policy_version_id',v_signal_policy
      )
    )
  );
  v_receipt:=public.mhidas_ticket_reserve_operation_receipt_v10('signal_insert','trusted_ticketing_integration',null,v_authority.principal_namespace,p_integration_id,p_credential_version_id,'record_purchase_signal_v12',p_idempotency_key,'purchase_signal',v_signal_id,null,v_request_hash,p_correlation_id,v_receipt_policy);
  if v_receipt.receipt_status='completed' then
  ...
  begin
    perform public.mhidas_ticket_consume_verified_credential_context_v7(p_credential_context_id,p_context_token,v_required_scope,'record_purchase_signal_v12',v_request_hash,v_receipt.receipt_id,v_signal_id,p_integration_id,p_credential_version_id);
...
create function public.mhidas_run_event_ticket_governance_retention_batch_v8(
...
declare
  ...
  v_signal_family_count integer:=0; v_signal_standalone_count integer:=0;
  v_failure_hash text; v_request_hash text;
begin
  if p_batch_limit not between 1 and 5000 then raise exception 'RETENTION_BATCH_LIMIT_INVALID_V8'; end if;
  v_receipt_policy:=public.mhidas_ticket_resolve_active_retention_policy_v2('operation_receipt','BR','operation_receipt');
  ...
  perform public.mhidas_ticket_assert_retention_execution_contract_v2(v_communication_policy,'partner_communication',5);
  perform public.mhidas_ticket_assert_retention_execution_contract_v2(v_backfill_policy,'backfill_rejection',6);
  ...
  if v_rule_count<>44 then raise exception 'RETENTION_MINIMIZATION_MATRIX_INCOMPLETE_V8'; end if;
  v_run_id:=public.mhidas_ticket_deterministic_uuid_v1('retention:'||p_idempotency_key);
  v_request_hash:=public.mhidas_ticket_sha256_v1(
    public.mhidas_ticket_canonical_payload_v1(
      'mhidas.retention-run.v7',
      jsonb_build_object(
        'batch_limit',p_batch_limit,
        'receipt_policy',v_receipt_policy,
        'audit_policy',v_audit_policy,
        'signal_policy',v_signal_policy,
        'click_policy',v_click_policy,
        'context_policy',v_context_policy,
        'communication_policy',v_communication_policy,
        'backfill_policy',v_backfill_policy
      )
    )
  );
  v_receipt:=public.mhidas_ticket_reserve_operation_receipt_v10('retention_run','service_role',null,'service_role:retention',null,null,'run_governance_retention_batch_v8',p_idempotency_key,'retention_run',v_run_id,null,v_request_hash,p_correlation_id,v_receipt_policy);
  ...
  begin
    update public.event_ticket_verified_credential_contexts
    set context_status='expired',expired_at=coalesce(expired_at,now()),last_lifecycle_receipt_id=v_receipt.receipt_id
    where context_status='active' and expires_at<=now();

    with c as (
      select r.receipt_id
      from public.event_ticket_operation_receipts r
      join public.event_ticket_retention_policy_versions p on p.retention_policy_version_id=r.receipt_retention_policy_version_id
      where p.policy_status in ('active','retired') and p.policy_purpose='operation_receipt' and p.jurisdiction_code='BR' and p.evidence_class='operation_receipt'
        and r.reserved_at<now()-make_interval(days=>p.retention_days) and r.minimized_at is null and r.receipt_id<>v_receipt.receipt_id
      order by r.reserved_at limit p_batch_limit for update of r skip locked
    )
    ...
    with c as (
      ...
      join public.event_ticket_retention_policy_versions p on p.retention_policy_version_id=a.audit_retention_policy_version_id
      where p.policy_status in ('active','retired') and p.policy_purpose='commercial_audit' and p.jurisdiction_code='BR' and p.evidence_class='commercial_audit'
        and a.created_at<now()-make_interval(days=>p.retention_days) and a.minimized_at is null
      order by a.created_at limit p_batch_limit for update of a skip locked
    )
    ...
    with family_due as (
      select s.transaction_family_id,count(*) as row_count,max(s.created_at) as newest_member_at,
        bool_and(s.created_at<now()-make_interval(days=>p.retention_days)) as every_member_due
      from public.event_ticket_purchase_signals s
      join public.event_ticket_retention_policy_versions p on p.retention_policy_version_id=s.retention_policy_version_id
      where p.policy_status in ('active','retired') and p.policy_purpose='event_ticket_tracking' and p.jurisdiction_code='BR'
        and p.evidence_class='purchase_signal' and s.minimized_at is null and s.transaction_family_id is not null
      group by s.transaction_family_id
    ), ranked as (
      select f.*,sum(f.row_count) over(order by f.newest_member_at,f.transaction_family_id) as cumulative_rows
      from family_due f where f.every_member_due
    ), selected_families as (
      select r.transaction_family_id from ranked r
      where r.cumulative_rows<=p_batch_limit
        and pg_try_advisory_xact_lock(hashtextextended(r.transaction_family_id::text,0))
    ), c as (
      select s.signal_id
      from public.event_ticket_purchase_signals s
      join selected_families f on f.transaction_family_id=s.transaction_family_id
      where s.minimized_at is null
      order by s.created_at,s.signal_id
      for update of s skip locked
    )
    update public.event_ticket_purchase_signals s
    set anonymized_transaction_hash=coalesce(s.anonymized_transaction_hash,case when s.external_transaction_hash is null then null else public.mhidas_ticket_sha256_v1('signal.family:'||s.integration_id::text||':'||s.external_transaction_hash) end),
        user_id=null,channel_id=null,click_id=null,credential_context_id=null,external_transaction_hash=null,
        evidence_metadata='{}'::jsonb,signal_status='tombstoned',tombstoned_at=coalesce(tombstoned_at,now()),minimized_at=now()
    from c where s.signal_id=c.signal_id;
    get diagnostics v_signal_family_count=row_count;

    with c as (
      select s.signal_id
      from public.event_ticket_purchase_signals s
      join public.event_ticket_retention_policy_versions p on p.retention_policy_version_id=s.retention_policy_version_id
      where p.policy_status in ('active','retired') and p.policy_purpose='event_ticket_tracking' and p.jurisdiction_code='BR' and p.evidence_class='purchase_signal'
        and s.created_at<now()-make_interval(days=>p.retention_days) and s.minimized_at is null and s.transaction_family_id is null
      order by s.created_at limit p_batch_limit for update of s skip locked
    )
    update public.event_ticket_purchase_signals s
    set user_id=null,channel_id=null,click_id=null,credential_context_id=null,external_transaction_hash=null,
        evidence_metadata='{}'::jsonb,signal_status='tombstoned',tombstoned_at=coalesce(tombstoned_at,now()),minimized_at=now()
    from c where s.signal_id=c.signal_id;
    get diagnostics v_signal_standalone_count=row_count;
    v_signal_count:=v_signal_family_count+v_signal_standalone_count;

    with c as (
      select a.click_id from public.event_ticket_click_attributions a
      join public.event_ticket_retention_policy_versions p on p.retention_policy_version_id=a.retention_policy_version_id
      where p.policy_status in ('active','retired') and p.policy_purpose='event_ticket_tracking' and p.jurisdiction_code='BR' and p.evidence_class='click_attribution'
        and a.created_at<now()-make_interval(days=>p.retention_days) and a.anonymized_at is null
      order by a.created_at limit p_batch_limit for update of a skip locked
    )
    ...
    with c as (
      select x.context_id from public.event_ticket_verified_credential_contexts x
      join public.event_ticket_retention_policy_versions p on p.retention_policy_version_id=x.retention_policy_version_id
      where p.policy_status in ('active','retired') and p.policy_purpose='security_audit' and p.jurisdiction_code='BR' and p.evidence_class='credential_context'
        and x.issued_at<now()-make_interval(days=>p.retention_days) and x.minimized_at is null and x.context_status<>'active'
      order by x.issued_at limit p_batch_limit for update of x skip locked
    )
    ...
    with c as (
      select m.communication_id from public.partner_official_communications m
      join public.event_ticket_retention_policy_versions p on p.retention_policy_version_id=m.retention_policy_version_id
      where p.policy_status in ('active','retired') and p.policy_purpose='commercial_governance' and p.jurisdiction_code='BR' and p.evidence_class='partner_communication'
        and m.created_at<now()-make_interval(days=>p.retention_days) and m.minimized_at is null
      order by m.created_at limit p_batch_limit for update of m skip locked
    )
    ...
    with c as (
      select b.rejection_id from public.event_ticket_backfill_rejections b
      join public.event_ticket_retention_policy_versions p on p.retention_policy_version_id=b.retention_policy_version_id
      where p.policy_status in ('active','retired') and p.policy_purpose='commercial_governance' and p.jurisdiction_code='BR' and p.evidence_class='backfill_rejection'
        and b.created_at<now()-make_interval(days=>p.retention_days) and b.minimized_at is null
      order by b.created_at limit p_batch_limit for update of b skip locked
    )
    ...
    update public.event_ticket_retention_policy_versions p
    set retirement_drain_completed_at=now(),updated_at=now()
    where p.policy_status='retired' and p.retirement_drain_completed_at is null
      and not exists (select 1 from public.event_ticket_operation_receipts r where r.receipt_retention_policy_version_id=p.retention_policy_version_id and r.minimized_at is null)
      and not exists (select 1 from public.event_ticket_commercial_audit_log a where a.audit_retention_policy_version_id=p.retention_policy_version_id and a.minimized_at is null)
      and not exists (select 1 from public.event_ticket_purchase_signals s where s.retention_policy_version_id=p.retention_policy_version_id and s.minimized_at is null)
      and not exists (select 1 from public.event_ticket_click_attributions c where c.retention_policy_version_id=p.retention_policy_version_id and c.anonymized_at is null)
      and not exists (select 1 from public.event_ticket_verified_credential_contexts x where x.retention_policy_version_id=p.retention_policy_version_id and x.minimized_at is null)
      and not exists (select 1 from public.partner_official_communications m where m.retention_policy_version_id=p.retention_policy_version_id and m.minimized_at is null)
      and not exists (select 1 from public.event_ticket_backfill_rejections b where b.retention_policy_version_id=p.retention_policy_version_id and b.minimized_at is null);
    ...
    perform public.mhidas_ticket_complete_operation_receipt_v6(v_receipt.receipt_id,'completed',1,public.mhidas_ticket_sha256_v1(v_run_id::text||':completed'));
...
begin
  ...
  begin
    ...
    set constraints event_ticket_integrations_current_credential_v4118_ct, event_ticket_credentials_current_invariant_v4118_ct immediate;
...
create function public.mhidas_admin_authorize_activate_commercial_channel_v3(
  p_channel_id uuid,
  p_expected_lock_version integer,
...
begin
  ...
  v_receipt:=public.mhidas_ticket_reserve_operation_receipt_v10('channel_mutation','user',auth.uid(),'auth:user',null,null,'authorize_activate_commercial_channel_v3',p_idempotency_key,'commercial_channel',p_channel_id,p_expected_lock_version,v_request_hash,p_correlation_id,v_receipt_policy);
...
create function public.mhidas_admin_prepare_event_ticket_legacy_backfill_v1(
  p_idempotency_key text,
  p_correlation_id text
)
returns jsonb
...
declare
  v_run_id uuid;
  v_intent_count bigint;
  v_request_count bigint;
  v_source_count bigint;
  v_inventory_hash text;
begin
  if auth.uid() is null or public.mhidas_is_useclubbers_admin_v1(auth.uid()) is distinct from true then raise exception 'USECLUBBERS_ADMIN_REQUIRED_BACKFILL_V1'; end if;
  select count(*) into v_intent_count from public.event_ticket_intents;
  select count(*) into v_request_count from public.partner_ticket_requests;
  select count(*) into v_source_count from public.event_sources;
  v_inventory_hash:=public.mhidas_ticket_sha256_v1(public.mhidas_ticket_canonical_payload_v1('mhidas.legacy-backfill.inventory.v1',jsonb_build_object('event_ticket_intents',v_intent_count,'partner_ticket_requests',v_request_count,'event_sources',v_source_count)));
  v_run_id:=public.mhidas_ticket_deterministic_uuid_v1('legacy-backfill:'||p_idempotency_key);
  ...
  values (v_run_id,p_idempotency_key,v_inventory_hash,0,0,public.mhidas_ticket_sha256_v1('mapped:empty'),public.mhidas_ticket_sha256_v1('rejected:empty'),auth.uid())
  on conflict (idempotency_key) do nothing;
  return jsonb_build_object('status','prepared','backfill_run_id',v_run_id,'source_inventory_hash',v_inventory_hash,'legacy_row_count',v_intent_count+v_request_count+v_source_count,'correlation_hash',public.mhidas_ticket_sha256_v1(p_correlation_id));
end;
$mhidas_plpgsql$;
...
create function public.mhidas_ticket_url_proof_is_fresh_v8(
  p_channel_id uuid,
...
as $mhidas_sql$
  with payload as (
    select public.mhidas_ticket_canonical_payload_v1(
      'mhidas.url-proof.v8',
      jsonb_build_object(
        'channel_id',p_channel_id,
        'destination_url_hash',p_destination_url_hash,
        'hostname',p_hostname,
        'validated_at_epoch_ms',floor(extract(epoch from p_validated_at)*1000)::bigint,
        'expires_at_epoch_ms',floor(extract(epoch from p_expires_at)*1000)::bigint,
        'last_health_checked_at_epoch_ms',floor(extract(epoch from p_last_health_checked_at)*1000)::bigint,
        'last_health_check_hash',p_last_health_check_hash,
        'validator_version',p_validator_version,
        'resolved_host_hash',p_resolved_host_hash,
        'resolved_ip_set_hash',p_resolved_ip_set_hash,
        'redirect_chain_validation_hash',p_redirect_chain_validation_hash
      )
    ) as canonical_payload
  )
  select coalesce((p_status='validated'
    and p_channel_id is not null
    and p_destination_url_hash ~ '^[0-9a-f]{64}$'
    ...
    and p_attestation_hash=public.mhidas_ticket_sha256_v1(p_detached_signature)
    and p_manifest_hash=public.mhidas_ticket_sha256_v1(payload.canonical_payload)
    and p_canonical_payload_hash=p_manifest_hash
    and exists (
      select 1 from public.event_ticket_url_validation_authorities a
      where a.url_validation_authority_id=p_authority_id and a.authority_status='active'
        ...
        and a.signer_key_hash=p_signer_key_hash and a.signer_key_id=p_signer_key_id
        and a.signature_algorithm=p_signature_algorithm
        and public.mhidas_verify_detached_signature_v1(a.signer_key_id,a.signature_algorithm,payload.canonical_payload,p_detached_signature) is true
    )), false)
  from payload;
$mhidas_sql$;

create function public.mhidas_ticket_official_source_proof_is_fresh_v1(
  p_canonical_event_id uuid,
  p_source_url text,
  p_source_url_hash text,
  p_source_hostname text,
  p_authority_manifest_hash text,
  p_canonical_payload_hash text,
  p_detached_signature text,
  p_attestation_signature_hash text,
  p_signer_key_hash_snapshot text,
  p_signer_key_id_snapshot text,
  p_signature_algorithm_snapshot text,
  p_validator_version integer,
  p_commercial_policy_status text,
  p_validated_at timestamptz,
  p_expires_at timestamptz,
  p_authority_id uuid
)
...
as $mhidas_sql$
  with payload as (
    select public.mhidas_ticket_canonical_payload_v1(
      'mhidas.official-source.v1',
      jsonb_build_object(
        'canonical_event_id',p_canonical_event_id,
        'source_url_hash',p_source_url_hash,
        'source_hostname',p_source_hostname,
        'authority_manifest_hash',p_authority_manifest_hash,
        'validator_version',p_validator_version,
        'commercial_policy_status',p_commercial_policy_status,
        'validated_at_epoch_ms',floor(extract(epoch from p_validated_at)*1000)::bigint,
        'expires_at_epoch_ms',floor(extract(epoch from p_expires_at)*1000)::bigint
      )
    ) as canonical_payload
  )
  select coalesce((p_source_url ~ '^https://'
    and p_source_url_hash=public.mhidas_ticket_sha256_v1(p_source_url)
    and p_source_hostname=public.mhidas_ticket_extract_hostname_v2(p_source_url)
    and public.mhidas_ticket_hostname_is_public_v2(p_source_hostname)
    and p_commercial_policy_status in ('official_noncommercial','commercial_authorized')
    and p_validated_at<=now() and p_expires_at>now() and p_expires_at<=p_validated_at+interval '30 days'
    and p_attestation_signature_hash=public.mhidas_ticket_sha256_v1(p_detached_signature)
    and p_canonical_payload_hash=public.mhidas_ticket_sha256_v1(payload.canonical_payload)
    and exists (
      select 1 from public.event_ticket_url_validation_authorities a
      where a.url_validation_authority_id=p_authority_id and a.authority_status='active'
        and a.valid_from<=now() and (a.valid_until is null or a.valid_until>now())
        and a.signer_key_hash=p_signer_key_hash_snapshot and a.signer_key_id=p_signer_key_id_snapshot
        and a.signature_algorithm=p_signature_algorithm_snapshot
        and public.mhidas_verify_detached_signature_v1(a.signer_key_id,a.signature_algorithm,payload.canonical_payload,p_detached_signature) is true
    )), false)
  from payload;
$mhidas_sql$;

create function public.mhidas_resolve_public_event_ticket_action_v9(p_canonical_event_id uuid)
...
as $mhidas_sql$
  ...
  left join lateral (
    ...
    where ces.canonical_event_id=e.canonical_event_id and ces.source_url ~ '^https://' and ces.last_seen_at>=now()-interval '30 days'
      and public.mhidas_ticket_official_source_proof_is_fresh_v1(
...
create policy commercial_partners_admin_read_v4118 on public.commercial_partners for select to authenticated using (public.mhidas_is_useclubbers_admin_v1(auth.uid()));
create policy event_ticket_channels_admin_read_v4118 on public.event_ticket_commercial_channels for select to authenticated using (public.mhidas_is_useclubbers_admin_v1(auth.uid()));
create policy event_ticket_requests_owner_or_admin_read_v4118 on public.event_ticket_partnership_requests for select to authenticated using (submitted_by_user_id=auth.uid() or public.mhidas_is_useclubbers_admin_v1(auth.uid()));
create policy event_ticket_signals_admin_read_v4118 on public.event_ticket_purchase_signals for select to authenticated using (public.mhidas_is_useclubbers_admin_v1(auth.uid()));
create policy event_ticket_audit_admin_read_v4118 on public.event_ticket_commercial_audit_log for select to authenticated using (public.mhidas_is_useclubbers_admin_v1(auth.uid()));
...
revoke all on function public.mhidas_ticket_json_matches_contract_v3(text,jsonb,integer) from PUBLIC,anon,authenticated,service_role;
...
revoke all on function public.mhidas_ticket_assert_current_credential_invariant_v1() from PUBLIC,anon,authenticated,service_role;
...
revoke all on function public.mhidas_ticket_issue_verified_credential_context_v7(uuid,uuid,text,text,text,text,text,text,text,text,timestamptz,text) from PUBLIC,anon,authenticated,service_role;
revoke all on function public.mhidas_ticket_consume_verified_credential_context_v7(uuid,text,text,text,text,uuid,uuid,uuid,uuid) from PUBLIC,anon,authenticated,service_role;
revoke all on function public.mhidas_record_event_ticket_purchase_signal_v12(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb,uuid,text,text) from PUBLIC,anon,authenticated,service_role;
revoke all on function public.mhidas_run_event_ticket_governance_retention_batch_v8(integer,text,text) from PUBLIC,anon,authenticated,service_role;
...
revoke all on function public.mhidas_admin_authorize_activate_commercial_channel_v3(uuid,integer,text,text,text,text,text) from PUBLIC,anon,authenticated,service_role;
revoke all on function public.mhidas_ticket_url_proof_is_fresh_v8(uuid,text,text,text,timestamptz,timestamptz,timestamptz,text,integer,text,text,text,text,text,text,text,text,text,text,uuid) from PUBLIC,anon,authenticated,service_role;
revoke all on function public.mhidas_ticket_official_source_proof_is_fresh_v1(uuid,text,text,text,text,text,text,text,text,text,text,integer,text,timestamptz,timestamptz,uuid) from PUBLIC,anon,authenticated,service_role;
revoke all on function public.mhidas_resolve_public_event_ticket_action_v9(uuid) from PUBLIC,anon,authenticated,service_role;
...
revoke all on function public.mhidas_ticket_verify_commercial_destination_envelope_v1(text,text,text,text,text) from PUBLIC,anon,authenticated,service_role;
...
revoke all on function public.mhidas_admin_prepare_event_ticket_legacy_backfill_v1(text,text) from PUBLIC,anon,authenticated,service_role;
...
grant execute on function public.mhidas_ticket_issue_verified_credential_context_v7(uuid,uuid,text,text,text,text,text,text,text,text,timestamptz,text) to service_role;
grant execute on function public.mhidas_record_event_ticket_purchase_signal_v12(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb,uuid,text,text) to service_role;
grant execute on function public.mhidas_run_event_ticket_governance_retention_batch_v8(integer,text,text) to service_role;
...
grant execute on function public.mhidas_admin_authorize_activate_commercial_channel_v3(uuid,integer,text,text,text,text,text) to authenticated;
grant execute on function public.mhidas_admin_prepare_event_ticket_legacy_backfill_v1(text,text) to authenticated;
grant execute on function public.mhidas_resolve_public_event_ticket_action_v9(uuid) to anon,authenticated;

-- =============================================================================
-- 13. ELEVENTH-CORRECTED STRUCTURAL SELF-CHECK
-- =============================================================================

do $mhidas_v4118_self_check$
declare v_count integer;
begin
  select count(*) into v_count from public.event_ticket_retention_executor_contracts where contract_status='active';
  if v_count<>7 then raise exception 'V4_8_118_RETENTION_EXECUTOR_CONTRACT_COUNT_INVALID'; end if;
  if to_regclass('public.event_ticket_legacy_backfill_runs') is null or to_regclass('public.event_ticket_legacy_cutover_checkpoints') is null then raise exception 'V4_8_118_LEGACY_BACKFILL_CONTRACTS_MISSING'; end if;
  if exists (select 1 from public.event_ticket_commercial_channels c where c.channel_status='active' and public.mhidas_ticket_verify_commercial_destination_envelope_v1(c.destination_url_ciphertext,c.destination_envelope_key_id,c.destination_envelope_mac,c.destination_url_hash,c.destination_hostname) is distinct from true) then raise exception 'V4_8_118_ACTIVE_CHANNEL_ENVELOPE_DRIFT'; end if;
  if exists (
    ...
    where n.nspname='public' and a.privilege_type='EXECUTE' and r.rolcanlogin
      and (p.proname like 'mhidas_ticket_%' or p.proname like 'mhidas_record_event_ticket_%' or p.proname like 'mhidas_admin_%' or p.proname like 'mhidas_resolve_public_event_ticket_%')
      and not (
        (r.rolname='service_role' and p.proname in ('mhidas_ticket_issue_verified_credential_context_v7','mhidas_record_event_ticket_purchase_signal_v12','mhidas_run_event_ticket_governance_retention_batch_v8'))
        or (r.rolname='authenticated' and p.proname in ('mhidas_admin_mutate_commercial_partner_status_v6','mhidas_admin_rotate_event_ticket_integration_credential_v2','mhidas_admin_retire_event_ticket_retention_policy_v7','mhidas_admin_authorize_activate_commercial_channel_v3','mhidas_admin_prepare_event_ticket_legacy_backfill_v1','mhidas_resolve_public_event_ticket_action_v9'))
        or (r.rolname='anon' and p.proname='mhidas_resolve_public_event_ticket_action_v9')
      )
  ) then raise exception 'V4_8_118_EFFECTIVE_FUNCTION_EXECUTE_ALLOWLIST_DRIFT'; end if;
  select count(*) into v_count from public.event_ticket_retention_policy_versions where policy_key like '%_br_template';
  if v_count<>7 then raise exception 'V4_8_118_RETENTION_POLICY_TEMPLATE_COUNT_INVALID'; end if;
  select count(*) into v_count from public.event_ticket_retention_policy_versions where policy_key like '%_br_template' and policy_status='active' and bootstrap_receipt_id='00000000-0000-4118-8000-000000000000'::uuid;
  if v_count<>7 then raise exception 'V4_8_118_RETENTION_POLICY_BOOTSTRAP_ACTIVE_COUNT_INVALID'; end if;
  select count(*) into v_count from public.event_ticket_retention_minimization_rules where rule_status='active';
  if v_count<>44 then raise exception 'V4_8_118_MINIMIZATION_RULE_COUNT_INVALID'; end if;
  select count(*) into v_count from public.event_ticket_operation_result_contracts where contract_status='active';
  if v_count<>9 then raise exception 'V4_8_118_RESULT_CONTRACT_COUNT_INVALID'; end if;
  if (select provolatile from pg_proc where oid='public.mhidas_ticket_resolve_active_retention_policy_v2(text,text,text)'::regprocedure)<>'v' then raise exception 'V4_8_118_RETENTION_RESOLVER_NOT_VOLATILE'; end if;
  if not exists (select 1 from pg_trigger where tgname='event_ticket_integrations_current_credential_v4118_ct' and tgdeferrable) then raise exception 'V4_8_118_INTEGRATION_CONSTRAINT_TRIGGER_MISSING'; end if;
  if exists (select 1 from public.event_ticket_trusted_integrations i where i.integration_status='active' and (i.current_credential_version_id is null or i.issuer_service_key_hash is null)) then raise exception 'V4_8_118_ACTIVE_INTEGRATION_AUTHORITY_DRIFT'; end if;
  if exists (select integration_id from public.event_ticket_trusted_integration_credential_versions where credential_status='active' group by integration_id having count(*)>1) then raise exception 'V4_8_118_MULTIPLE_ACTIVE_CREDENTIALS'; end if;
  if exists (select 1 from public.event_ticket_commercial_channels c where c.channel_status='active' and (c.authorization_receipt_id is null or c.financial_terms_hash is null or c.lifecycle_snapshot_hash is null or c.authorized_scope_snapshot_hash is null or c.url_validation_authority_id is null or c.url_proof_signature is null or c.destination_url_ciphertext is null or c.valid_from is null)) then raise exception 'V4_8_118_ACTIVE_CHANNEL_AUTHORITY_DRIFT'; end if;
  if exists (select 1 from public.event_ticket_purchase_signals s where s.external_transaction_hash is not null and s.transaction_family_id is null) then raise exception 'V4_8_118_SIGNAL_FAMILY_ID_DRIFT'; end if;
  if exists (select 1 from public.event_ticket_operation_receipts r where r.receipt_status='completed' and not exists (select 1 from public.event_ticket_operation_result_contracts c where c.operation_name=r.operation_name and c.result_status=r.result_status and c.contract_status='active' and r.result_version>=c.minimum_result_version and (c.maximum_result_version is null or r.result_version<=c.maximum_result_version))) then raise exception 'V4_8_118_RECEIPT_RESULT_CONTRACT_DRIFT'; end if;
  if exists (
  ...
  ) then raise exception 'V4_8_118_RETIRED_POLICY_SUCCESSOR_DRIFT'; end if;
  if exists (
    select 1
    from unnest(array[
      'commercial_partners','commercial_partner_representatives','event_ticket_trusted_integrations',
  ...
  ) then raise exception 'V4_8_118_SERVICE_ROLE_EFFECTIVE_DML_GRANT_DRIFT'; end if;
  ...
  if v_count<>5 then raise exception 'V4_8_118_RLS_SELECT_GRANT_COUNT_INVALID'; end if;
  if position('destination_url_ciphertext' in pg_get_function_result('public.mhidas_resolve_public_event_ticket_action_v9(uuid)'::regprocedure))>0 then raise exception 'V4_8_118_PUBLIC_RESOLVER_EXPOSES_CIPHERTEXT'; end if;
  if exists (
    select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public'
      ...
      and (p.proname like 'mhidas_ticket_%' or p.proname like 'mhidas_record_event_ticket_%' or p.proname like 'mhidas_admin_%' or p.proname like 'mhidas_resolve_public_event_ticket_%')
      and p.oid not in (
        'public.mhidas_ticket_issue_verified_credential_context_v7(uuid,uuid,text,text,text,text,text,text,text,text,timestamptz,text)'::regprocedure,
        'public.mhidas_record_event_ticket_purchase_signal_v12(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb,uuid,text,text)'::regprocedure,
        'public.mhidas_run_event_ticket_governance_retention_batch_v8(integer,text,text)'::regprocedure
      )
  ) then raise exception 'V4_8_118_SERVICE_ROLE_PRIVILEGED_ROUTINE_ALLOWLIST_DRIFT'; end if;
end;
$mhidas_v4118_self_check$;

-- =============================================================================
-- 14. EXPLICIT PROTECTED-DRAFT DECISION
-- =============================================================================

-- parsed_postgres_statement_count=146
-- draft_decision=twelfth_corrected_adjusted_draft_ready_for_thirteenth_structural_review
-- normalized_schema_from_base=True
-- corrected_adjustments=10
-- critical_corrections=6
-- high_corrections=4
-- retention_signal_family_due_and_row_bounded=True
-- concurrent_receipt_semantics_complete=True
-- credential_signature_registered_hash_bound=True
-- commercial_destination_envelope_url_bound=True
-- conversion_confirmation_scope_enforced=True
-- expired_channel_atomic_replacement=True
-- retention_executor_external_contract=True
-- legacy_backfill_cutover_contract_materialized=True
-- contract_json_scalar_types_closed=True
-- effective_function_execute_allowlist_closed=True
...
-- v4_8_116_eleventh_corrected_sql_changed=False
