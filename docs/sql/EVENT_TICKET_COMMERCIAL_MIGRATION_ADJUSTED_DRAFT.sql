-- docs/sql/EVENT_TICKET_COMMERCIAL_MIGRATION_ADJUSTED_DRAFT.sql
-- Version: v4.8.90-event-ticket-commercial-migration-adjusted-draft-safe
-- Base: v4.8.89-event-ticket-commercial-migration-adjustment-plan-safe
--
...
-- - prior v4.8.87 SQL remains preserved unchanged.
...
begin
  raise exception using
    errcode = 'P0001',
    message = 'MHIDAS_PROTECTED_DRAFT_V4_8_90',
...
begin
  ...
  if to_regprocedure('public.mhidas_admin_mutate_event_ticket_commercial_channel_v1(text,uuid,integer,jsonb,text,text,text,boolean)') is not null then
...
create function public.mhidas_ticket_metadata_is_safe_v1(p_value jsonb)
returns boolean
language plpgsql
immutable
strict
  ...
  if jsonb_typeof(p_value) = 'object' then
    ...
    loop
      if lower(v_key) = any(array[
        'ip','ip_address','raw_ip','raw_url','url','commercial_url','ticket_url',
        'secret','token','authorization','cookie','email','phone','whatsapp',
        'transaction_id','external_transaction_id','document','cpf','cnpj'
      ]) or lower(v_key) ~ '(secret|token|password|credential|raw_|transaction_id|ip_address)' then
        return false;
      end if;

      if jsonb_typeof(v_child) in ('object', 'array')
        and not public.mhidas_ticket_metadata_is_safe_v1(v_child) then
        return false;
      end if;
    end loop;
...
create table public.commercial_partners (
  ...
  constraint commercial_partners_metadata_v490_check
    check (
      ...
      and public.mhidas_ticket_metadata_is_safe_v1(metadata)
    )
);
...
create table public.event_ticket_retention_policy_versions (
  ...
  constraint event_ticket_retention_policy_actions_v490_check
    check (
      ...
      and signal_action in (
        'delete_non_conversion_anonymize_conversion',
        'anonymize_all'
      )
    ),
...
create table public.event_ticket_retention_runs (
  ...
  signal_deleted_count integer not null default 0,
  signal_anonymized_count integer not null default 0,
  ...
  constraint event_ticket_retention_runs_counts_v490_check
    check (
      ...
      and signal_deleted_count >= 0
      and signal_anonymized_count >= 0
    ),
  constraint event_ticket_retention_runs_checkpoint_v490_check
    check (
      jsonb_typeof(checkpoint) = 'object'
      and public.mhidas_ticket_metadata_is_safe_v1(checkpoint)
    ),
    ...
    check (
      (
        ...
        or (
          ...
          and public.mhidas_ticket_metadata_is_safe_v1(before_snapshot)
        )
      )
      and (
        ...
        or (
          ...
          and public.mhidas_ticket_metadata_is_safe_v1(after_snapshot)
        )
      )
    ),
...
create table public.event_ticket_operation_receipts (
  ...
  created_at timestamptz not null default now(),
  constraint event_ticket_operation_receipts_scope_v490_check
    check (operation_scope in (
      'request_submit','request_mutation','channel_mutation',
      'communication_submit','communication_mutation','signal_insert',
      'retention_run','channel_expiry'
    )),
  constraint event_ticket_operation_receipts_hash_v490_check
    check (result_hash ~ '^[0-9a-f]{64}$'),
  constraint event_ticket_operation_receipts_v490_unique
...
create function public.mhidas_ticket_partnership_request_guard_v1()
returns trigger
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  if tg_op = 'INSERT' then
    if v_actor_role = 'verified_partner_representative' then
      ...
      if v_partner_id is null or v_partner_id <> new.partner_id then
...
on public.event_ticket_partnership_requests
for each row
execute function public.mhidas_ticket_partnership_request_guard_v1();

create function public.mhidas_ticket_commercial_channel_guard_v1()
returns trigger
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  if v_actor_role <> 'useclubbers_admin'
    and not (
      v_actor_role = 'automation'
      and tg_op = 'UPDATE'
      and old.channel_status = 'active'
      and new.channel_status = 'expired'
    ) then
  ...
  if new.source_origin = 'approved_partner_request' then
    ...
    if not found then
      raise exception 'COMMERCIAL_CHANNEL_SOURCE_REQUEST_NOT_FOUND';
    end if;
...
on public.event_ticket_commercial_channels
for each row
execute function public.mhidas_ticket_commercial_channel_guard_v1();

create function public.mhidas_ticket_purchase_signal_append_only_v1()
returns trigger
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  if tg_op = 'DELETE'
    ...
    and old.retention_expires_at <= now()
    and old.signal_type not in ('attributed_conversion','confirmed_conversion') then
    return old;
  end if;
  ...
  if tg_op = 'UPDATE'
    ...
    and old.signal_id = new.signal_id
    and old.parent_signal_id is not distinct from new.parent_signal_id
    and old.canonical_event_id = new.canonical_event_id
    and old.channel_id is not distinct from new.channel_id
    and old.click_id is not distinct from new.click_id
    and old.signal_type = new.signal_type
    and old.evidence_source = new.evidence_source
    and old.transaction_hash is not distinct from new.transaction_hash
    and old.evidence_hash is not distinct from new.evidence_hash
    and old.idempotency_key = new.idempotency_key then
    return new;
  end if;
...
on public.event_ticket_purchase_signals
for each row
execute function public.mhidas_ticket_purchase_signal_append_only_v1();
...
create function public.mhidas_partner_submit_ticket_partnership_request_v1(
  p_canonical_event_id uuid,
  p_partner_id uuid,
  p_partner_type text,
  p_request_type text,
...
begin
  ...
  where operation_scope = 'request_submit'
  ...
  if found then
    ...
    where request_id = v_receipt.target_id;
    return v_request;
  end if;
  ...
  insert into public.event_ticket_partnership_requests (
    ...
    state_snapshot,
    venue_name_snapshot,
    current_sales_url_hash,
    proposed_benefit,
    commercial_contact_reference_id,
    commercial_notes_hash,
  ...
  values (
    ...
    p_partner_type,
    p_request_type,
    p_event_slug_snapshot,
    p_event_name_snapshot,
    p_event_date_snapshot,
    p_city_snapshot,
    p_state_snapshot,
    p_venue_name_snapshot,
    p_current_sales_url_hash,
    p_proposed_benefit,
    p_commercial_contact_reference_id,
    p_commercial_notes_hash,
  ...
  perform public.mhidas_ticket_write_audit_v1(
    ...
    v_request.commercial_notes_hash,
    p_reason,
    p_correlation_id,
    v_idempotency_key
  );
...
create function public.mhidas_mutate_ticket_partnership_request_v1(
  p_operation text,
  p_request_id uuid,
...
begin
  ...
  into v_receipt
  from public.event_ticket_operation_receipts
  where operation_scope = 'request_mutation'
  ...
  perform public.mhidas_ticket_write_audit_v1(
    ...
    null,
    null,
    'request_' || p_operation,
    v_before.request_status,
    v_request.request_status,
    v_request.lock_version,
    jsonb_build_object(
      'request_status', v_before.request_status,
      'lock_version', v_before.lock_version
    ),
    jsonb_build_object(
      'request_status', v_request.request_status,
      'lock_version', v_request.lock_version
    ),
    p_admin_notes_hash,
...
create function public.mhidas_admin_mutate_event_ticket_commercial_channel_v1(
  p_operation text,
  p_channel_id uuid,
  p_expected_lock_version integer,
  p_payload jsonb,
...
begin
  ...
  into v_receipt
  from public.event_ticket_operation_receipts
  where operation_scope = 'channel_mutation'
    and idempotency_key = p_idempotency_key;
  ...
  if found then
    ...
    where channel_id = v_receipt.target_id;
    return v_channel;
  end if;
  ...
  if p_operation = 'create_draft' then
    ...
    values (
      ...
      lower(p_payload ->> 'validated_hostname'),
      lower(p_payload ->> 'validated_final_hostname'),
      coalesce(p_payload ->> 'url_validation_status', 'pending'),
      nullif(p_payload ->> 'url_validation_algorithm', ''),
      nullif(p_payload ->> 'url_validation_version', '')::integer,
      nullif(p_payload ->> 'url_validation_hash', ''),
      nullif(p_payload ->> 'dns_public_validation_hash', ''),
      nullif(p_payload ->> 'redirect_chain_validation_hash', ''),
      nullif(p_payload ->> 'validated_final_url_hash', ''),
      nullif(p_payload ->> 'url_validated_at', '')::timestamptz,
      nullif(p_payload ->> 'url_validated_by_service', ''),
    ...
    if p_require_fresh_url
      and (
        v_before.url_validation_status <> 'validated'
        or v_before.url_validated_at is null
        or v_before.url_validated_at < now() - interval '24 hours'
      ) then
      raise exception 'CHANNEL_FRESH_URL_VALIDATION_REQUIRED';
    end if;
  ...
  values (
    ...
    encode(digest(
      v_channel.channel_id::text || ':' ||
      v_channel.channel_status || ':' ||
      v_channel.lock_version::text || ':' ||
...
create function public.mhidas_mutate_partner_official_communication_v1(
  p_operation text,
  p_communication_id uuid,
  p_expected_lock_version integer,
  p_payload jsonb,
...
begin
  ...
  into v_receipt
  from public.event_ticket_operation_receipts
  where operation_scope = 'communication_mutation'
    and idempotency_key = p_idempotency_key;
  ...
  if found then
    ...
    where communication_id = v_receipt.target_id;
  ...
  if p_operation = 'create_draft' then
    ...
    if v_actor_role = 'verified_partner_representative'
      and not exists (
        select 1
        from public.commercial_partner_representatives r
        where r.partner_id = v_before.partner_id
          and r.user_id = v_user_id
          and r.representation_status = 'active'
          and (r.valid_from is null or r.valid_from <= now())
          and (r.valid_until is null or r.valid_until > now())
...
create function public.mhidas_record_event_ticket_purchase_signal_v1(
  p_signal_id uuid,
...
begin
  ...
  into v_receipt
  from public.event_ticket_operation_receipts
  where operation_scope = 'signal_insert'
    and idempotency_key = p_idempotency_key;
  ...
  if found then
    ...
    where signal_id = v_receipt.target_id;
    return v_signal;
  end if;
...
create function public.mhidas_expire_event_ticket_commercial_channels_v1(
...
begin
  ...
  loop
    ...
    perform public.mhidas_ticket_write_audit_v1(
      ...
      jsonb_build_object(
        'channel_status', 'expired',
        'lock_version', v_channel.lock_version + 1
      ),
      v_channel.financial_terms_hash,
      'authorization validity ended',
      p_correlation_id,
...
create function public.mhidas_run_event_ticket_retention_batch_v1(
  p_retention_policy_version_id uuid,
  p_batch_limit integer,
  p_correlation_id text,
  p_idempotency_key text
...
declare
  ...
  v_signal_deleted integer := 0;
  v_signal_anonymized integer := 0;
begin
  if current_setting('request.jwt.claim.role', true) <> 'service_role' then
    raise exception 'SERVICE_ROLE_REQUIRED';
  end if;
  ...
  if not found then
    raise exception 'ACTIVE_RETENTION_POLICY_REQUIRED';
  end if;
  ...
  if v_policy.signal_action = 'delete_non_conversion_anonymize_conversion' then
    with candidates as (
      select s.signal_id
      from public.event_ticket_purchase_signals s
      where s.retention_expires_at <= now()
        and s.retention_processed_at is null
        and s.signal_type not in ('attributed_conversion','confirmed_conversion')
      order by s.retention_expires_at, s.signal_id
      for update skip locked
      limit p_batch_limit
    )
    delete from public.event_ticket_purchase_signals s
    using candidates x
    where s.signal_id = x.signal_id;

    get diagnostics v_signal_deleted = row_count;
  end if;
  ...
  set user_id = null,
      ...
      metadata = '{}'::jsonb,
      retention_processed_at = now(),
      retention_result = 'anonymized'
  from candidates x
  where s.signal_id = x.signal_id;

  get diagnostics v_signal_anonymized = row_count;
  ...
  set run_status = 'completed',
      ...
      signal_deleted_count = v_signal_deleted,
      signal_anonymized_count = v_signal_anonymized,
...
create function public.mhidas_resolve_public_event_ticket_action_v1(
  p_canonical_event_id uuid
)
returns table (
  action_kind text,
...
begin
  ...
  where c.canonical_event_id = p_canonical_event_id
    ...
    and c.url_validation_status = 'validated'
  ...
  if found then
    ...
    select
      ...
      v_channel.channel_id,
      null::uuid,
      'authorized_monetized_channel_active'::text;
    return;
  end if;
  ...
  if found then
    ...
    select
      ...
      null::uuid,
      v_source.id,
      'validated_official_reference_available'::text;
    return;
  end if;
  ...
  select
    ...
    null::uuid,
    null::uuid,
    'no_authorized_channel_or_validated_reference'::text;
end;
$mhidas_plpgsql$;
...
revoke all on function public.mhidas_ticket_domain_matches_v1(text,text,boolean)
  from public, anon, authenticated;
revoke all on function public.mhidas_ticket_metadata_is_safe_v1(jsonb)
  from public, anon, authenticated;
...
revoke all on function public.mhidas_partner_submit_ticket_partnership_request_v1(
  uuid,uuid,text,text,text,text,date,text,text,text,text,text,text,text,text,jsonb,text,text
) from public, anon, authenticated;
revoke all on function public.mhidas_mutate_ticket_partnership_request_v1(
  text,uuid,integer,text,text,text,text,text
) from public, anon, authenticated;
revoke all on function public.mhidas_admin_mutate_event_ticket_commercial_channel_v1(
  text,uuid,integer,jsonb,text,text,text,boolean
) from public, anon, authenticated;
revoke all on function public.mhidas_mutate_partner_official_communication_v1(
  text,uuid,integer,jsonb,text,text,text,text
) from public, anon, authenticated;
revoke all on function public.mhidas_record_event_ticket_purchase_signal_v1(
...
revoke all on function public.mhidas_expire_event_ticket_commercial_channels_v1(
  integer,text,text
) from public, anon, authenticated;
revoke all on function public.mhidas_run_event_ticket_retention_batch_v1(
  uuid,integer,text,text
) from public, anon, authenticated;
revoke all on function public.mhidas_resolve_public_event_ticket_action_v1(uuid)
  from public;

grant execute on function public.mhidas_partner_submit_ticket_partnership_request_v1(
  uuid,uuid,text,text,text,text,date,text,text,text,text,text,text,text,text,jsonb,text,text
) to authenticated;
grant execute on function public.mhidas_mutate_ticket_partnership_request_v1(
  text,uuid,integer,text,text,text,text,text
) to authenticated;
grant execute on function public.mhidas_admin_mutate_event_ticket_commercial_channel_v1(
  text,uuid,integer,jsonb,text,text,text,boolean
) to authenticated;
grant execute on function public.mhidas_mutate_partner_official_communication_v1(
  text,uuid,integer,jsonb,text,text,text,text
) to authenticated;
grant execute on function public.mhidas_record_event_ticket_purchase_signal_v1(
  uuid,uuid,uuid,uuid,uuid,uuid,text,text,boolean,text,text,text,text,text,text,bigint,bigint,text,timestamptz,uuid,timestamptz,text,jsonb,text
) to authenticated, service_role;
grant execute on function public.mhidas_expire_event_ticket_commercial_channels_v1(
  integer,text,text
) to service_role;
grant execute on function public.mhidas_run_event_ticket_retention_batch_v1(
  uuid,integer,text,text
) to service_role;
grant execute on function public.mhidas_resolve_public_event_ticket_action_v1(uuid)
...
(
  ...
  'implemented_in_protected_draft',
  array['mhidas_admin_mutate_event_ticket_commercial_channel_v1','operation receipts','audit writer'],
...
(
  ...
  'implemented_in_protected_draft',
  array['mhidas_ticket_partnership_request_guard_v1','mhidas_mutate_ticket_partnership_request_v1'],
...
(
  ...
  'implemented_in_protected_draft',
  array['atomic_cutover','single active channel index','mhidas_expire_event_ticket_commercial_channels_v1'],
...
(
  ...
  'implemented_in_protected_draft',
  array['SHA-256 format checks','hash version columns','recursive metadata denylist'],
...
(
  ...
  'implemented_in_protected_draft',
  array['retention policy versions','retention runs','retention batch RPC'],
