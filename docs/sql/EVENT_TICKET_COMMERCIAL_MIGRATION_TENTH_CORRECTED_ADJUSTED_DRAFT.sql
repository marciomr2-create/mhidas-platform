-- docs/sql/EVENT_TICKET_COMMERCIAL_MIGRATION_TENTH_CORRECTED_ADJUSTED_DRAFT.sql
-- Version: v4.8.114-event-ticket-commercial-migration-tenth-corrected-adjusted-draft-safe
-- Base: v4.8.113-event-ticket-commercial-migration-ninth-corrected-adjusted-draft-structural-review-safe
--
...
do $mhidas_guard$
begin
  raise exception 'MHIDAS_PROTECTED_DRAFT_V4_8_114';
end;
$mhidas_guard$;
...
begin
  ...
  for v_item in
    ...
    where n.nspname = 'public'
      and p.proname = any(array[
        'mhidas_ticket_sha256_v1','mhidas_ticket_hmac_sha256_v1','mhidas_ticket_canonical_payload_v1',
        'mhidas_ticket_deterministic_uuid_v1','mhidas_ticket_json_matches_contract_v2',
        'mhidas_ticket_extract_hostname_v2','mhidas_ticket_hostname_is_public_v2',
        'mhidas_ticket_resolve_active_retention_policy_v2','mhidas_ticket_lock_current_credential_v3',
        'mhidas_ticket_assert_current_credential_invariant_v1','mhidas_ticket_reserve_operation_receipt_v9',
        'mhidas_ticket_complete_operation_receipt_v6','mhidas_ticket_fail_operation_receipt_v6',
        'mhidas_ticket_issue_verified_credential_context_v6','mhidas_ticket_consume_verified_credential_context_v7',
        'mhidas_record_event_ticket_purchase_signal_v11','mhidas_run_event_ticket_governance_retention_batch_v7',
        'mhidas_admin_mutate_commercial_partner_status_v6','mhidas_admin_rotate_event_ticket_integration_credential_v2',
        'mhidas_admin_retire_event_ticket_retention_policy_v7','mhidas_admin_authorize_activate_commercial_channel_v2',
        'mhidas_ticket_url_proof_is_fresh_v8','mhidas_ticket_official_source_proof_is_fresh_v1',
        'mhidas_resolve_public_event_ticket_action_v8'
      ])
  loop
  ...
  for v_item in
    ...
    where schemaname = 'public' and policyname = any(array[
      'commercial_partners_admin_read_v4114',
      'event_ticket_channels_admin_read_v4114',
      'event_ticket_requests_owner_or_admin_read_v4114',
      'event_ticket_signals_admin_read_v4114',
      'event_ticket_audit_admin_read_v4114'
    ])
  loop
  ...
  for v_item in
    ...
    where not t.tgisinternal and n.nspname='public' and t.tgname = any(array[
      'event_ticket_integrations_current_credential_v4114_ct',
      'event_ticket_credentials_current_invariant_v4114_ct'
    ])
  loop
  ...
  foreach v_name in array array[
    'event_ticket_credentials_one_active_v4114_uq',
    'event_ticket_retention_policy_active_dimension_v4114_uq',
    'event_ticket_channels_one_active_v4114_uq',
    'event_ticket_receipts_semantic_v4114_uq',
    'event_ticket_signal_transaction_v4114_uq'
  ] loop
  ...
  if cardinality(v_missing) > 0 or cardinality(v_drift) > 0 then
    raise exception 'V4_8_114_BASE_PREFLIGHT_FAILED missing=[%] drift=[%]', array_to_string(v_missing, ','), array_to_string(v_drift, ',');
  end if;
end;
$mhidas_preflight$;
...
create table public.commercial_partner_representatives (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint commercial_partner_representatives_identity_v4114_unique
    unique (partner_id, user_id),
  constraint commercial_partner_representatives_role_v4114_check
    check (representative_role in ('owner','commercial','marketing','operations','legal')),
  constraint commercial_partner_representatives_status_v4114_check
    check (representation_status in ('pending','active','suspended','revoked')),
  constraint commercial_partner_representatives_validity_v4114_check
    check (valid_until is null or valid_from is null or valid_until > valid_from),
  constraint commercial_partner_representatives_metadata_v4114_check
...
create table public.event_ticket_trusted_integrations (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_integrations_key_v4114_check
    check (integration_key ~ '^[a-z0-9][a-z0-9_.-]{2,79}$'),
  constraint event_ticket_integrations_namespace_v4114_check
    check (principal_namespace ~ '^[a-z0-9][a-z0-9_.-]{2,79}$'),
  constraint event_ticket_integrations_status_v4114_check
    check (integration_status in ('pending','active','suspended','retired')),
  constraint event_ticket_integrations_partner_v4114_check
  ...
  constraint event_ticket_integrations_metadata_v4114_check
...
create table public.event_ticket_trusted_integration_credential_versions (
  ...
  created_at timestamptz not null default now(),
  constraint event_ticket_credentials_pair_v4114_unique
    unique (integration_id, credential_version_id),
  constraint event_ticket_credentials_version_v4114_unique
    unique (integration_id, credential_version),
  constraint event_ticket_credentials_status_v4114_check
    check (credential_status in ('pending','active','rotating','revoked','expired')),
  constraint event_ticket_credentials_hashes_v4114_check
    check (
  ...
  constraint event_ticket_credentials_validity_v4114_check
    check (valid_until is null or valid_until > valid_from)
);

create unique index event_ticket_credentials_one_active_v4114_uq
...
alter table public.event_ticket_trusted_integrations
  add constraint event_ticket_integrations_current_credential_v4114_fk
...
create constraint trigger event_ticket_integrations_current_credential_v4114_ct
...
create constraint trigger event_ticket_credentials_current_invariant_v4114_ct
...
create table public.event_ticket_trusted_integration_channels (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_integration_channels_identity_v4114_unique
    unique (integration_id, canonical_event_id, channel_scope),
  constraint event_ticket_integration_channels_authority_v4114_unique
    unique (integration_channel_id, integration_id, canonical_event_id),
  constraint event_ticket_integration_channels_authority_scope_v4114_unique
    unique (integration_channel_id, integration_id, canonical_event_id, channel_scope),
  constraint event_ticket_integration_channels_scope_v4114_check
    check (channel_scope in ('purchase_signal','url_validation','conversion_confirmation')),
  constraint event_ticket_integration_channels_status_v4114_check
    check (scope_status in ('pending','active','suspended','revoked')),
  constraint event_ticket_integration_channels_validity_v4114_check
    check (valid_until is null or valid_from is null or valid_until > valid_from)
);
...
create table public.event_ticket_partnership_requests (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_requests_submission_v4114_unique
    unique (partner_id, client_submission_key),
  constraint event_ticket_requests_authorization_v4114_unique
    unique (request_id, canonical_event_id, partner_id),
  constraint event_ticket_requests_type_v4114_check
  ...
  constraint event_ticket_requests_status_v4114_check
    check (request_status in ('pending','needs_info','approved','rejected','withdrawn')),
  constraint event_ticket_requests_hashes_v4114_check
    check (
  ...
  constraint event_ticket_requests_metadata_v4114_check
...
create table public.event_ticket_retention_policy_versions (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_retention_policy_version_v4114_unique
    unique (policy_key, policy_version),
  constraint event_ticket_retention_policy_status_v4114_check
    check (policy_status in ('draft','approved','active','retired')),
  constraint event_ticket_retention_policy_dimensions_v4114_check
    check (
      policy_purpose in (
  ...
  constraint event_ticket_retention_policy_action_v4114_check
    check (retention_action in ('delete','anonymize','tombstone')),
  constraint event_ticket_retention_policy_days_v4114_check
    check (retention_days between 1 and 3650),
  constraint event_ticket_retention_policy_hashes_v4114_check
    check (
      legal_basis_reference_hash ~ '^[0-9a-f]{64}$'
      and policy_manifest_hash ~ '^[0-9a-f]{64}$'
    ),
  constraint event_ticket_retention_policy_approval_v4114_check
    check (
      policy_status = 'draft'
      or (approved_by_admin_user_id is not null and approved_at is not null)
    ),
  constraint event_ticket_retention_policy_retirement_v4114_check
    check (
...
alter table public.event_ticket_retention_policy_versions
  add constraint event_ticket_retention_policy_successor_v4114_fk
...
create unique index event_ticket_retention_policy_active_dimension_v4114_uq
...
create table public.event_ticket_retention_minimization_rules (
  ...
  created_at timestamptz not null default now(),
  constraint event_ticket_minimization_rule_identity_v4114_unique
    unique (retention_policy_version_id, evidence_class, source_table, source_field),
  constraint event_ticket_minimization_rule_action_v4114_check
    check (minimization_action in ('delete','nullify','stable_hash','retain','replace_with_tombstone')),
  constraint event_ticket_minimization_rule_hash_v4114_check
    check (
      minimization_action <> 'stable_hash'
      or stable_hash_namespace ~ '^[a-z0-9][a-z0-9._-]{2,80}$'
    ),
  constraint event_ticket_minimization_rule_status_v4114_check
...
create table public.event_ticket_url_validation_authorities (
  ...
  approved_at timestamptz,
  created_at timestamptz not null default now(),
  constraint event_ticket_url_authority_status_v4114_check check (authority_status in ('pending','active','suspended','retired')),
  constraint event_ticket_url_authority_hash_v4114_check check (
    signer_key_hash ~ '^[0-9a-f]{64}$'
    and signer_key_id ~ '^[a-zA-Z0-9][a-zA-Z0-9._:/-]{2,199}$'
    and signature_algorithm in ('ed25519','ecdsa-p256-sha256','rsa-pss-sha256')
  ),
  constraint event_ticket_url_authority_validity_v4114_check check (valid_until is null or valid_until > valid_from)
);
...
create table public.event_ticket_official_source_attestations (
  ...
  created_at timestamptz not null default now(),
  constraint event_ticket_official_source_identity_v4114_unique unique (canonical_event_id, source_url),
  constraint event_ticket_official_source_hashes_v4114_check check (
  ...
  constraint event_ticket_official_source_signature_v4114_check check (
  ...
  constraint event_ticket_official_source_host_v4114_check check (
  ...
  constraint event_ticket_official_source_policy_v4114_check check (commercial_policy_status in ('official_noncommercial','commercial_authorized')),
  constraint event_ticket_official_source_freshness_v4114_check check (expires_at > validated_at and expires_at <= validated_at + interval '30 days')
);
...
create table public.event_ticket_commercial_channels (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_channels_semantic_v4114_unique unique (channel_id, canonical_event_id, integration_id),
  constraint event_ticket_channels_request_v4114_fk foreign key (request_id,canonical_event_id,partner_id)
    references public.event_ticket_partnership_requests(request_id,canonical_event_id,partner_id) on delete restrict,
  constraint event_ticket_channels_scope_v4114_fk foreign key (integration_channel_id,integration_id,canonical_event_id)
  ...
  constraint event_ticket_channels_status_v4114_check check (channel_status in ('draft','authorized','active','paused','expired','revoked')),
  constraint event_ticket_channels_kind_v4114_check check (channel_kind in ('official_reference','affiliate','discount','presale','fixed_media','hybrid')),
  constraint event_ticket_channels_tracking_v4114_check check (tracking_method in ('none','redirect','coupon','postback','webhook','partner_api')),
  constraint event_ticket_channels_financial_v4114_check check (
  ...
  constraint event_ticket_channels_hashes_v4114_check check (
  ...
  constraint event_ticket_channels_active_authority_v4114_check check (
    channel_status <> 'active' or (
      ...
      and destination_url_hash is not null and url_validation_status='validated' and url_validation_authority_id is not null
      ...
      and url_proof_signature_algorithm is not null and resolved_ip_set_hash is not null
    )
  ),
  constraint event_ticket_channels_metadata_v4114_check check (public.mhidas_ticket_json_matches_contract_v2('channel_metadata', metadata))
);

create unique index event_ticket_channels_one_active_v4114_uq
...
create table public.event_ticket_operation_result_contracts (
  ...
  constraint event_ticket_result_contract_name_v4114_check check (operation_name ~ '^[a-z0-9][a-z0-9._:-]{2,119}$'),
  constraint event_ticket_result_contract_status_v4114_check check (
    result_status ~ '^[a-z0-9][a-z0-9._:-]{2,79}$'
    and result_status not in ('pending','failed')
    and contract_status in ('active','retired')
  ),
  constraint event_ticket_result_contract_version_v4114_check check (
...
create table public.event_ticket_operation_receipts (
  ...
  constraint event_ticket_receipts_credential_pair_v4114_fk
    ...
    on delete restrict,
  constraint event_ticket_receipts_scope_v4114_check
  ...
  constraint event_ticket_receipts_principal_v4114_check
    check (
  ...
  constraint event_ticket_receipts_target_v4114_check
  ...
  constraint event_ticket_receipts_hashes_v4114_check
    check (
  ...
  constraint event_ticket_receipts_state_v4114_check
    check (
...
create unique index event_ticket_receipts_semantic_v4114_uq
...
alter table public.event_ticket_commercial_channels
  add constraint event_ticket_channels_authorization_receipt_v4114_fk
...
create table public.event_ticket_verified_credential_contexts (
  ...
  minimized_at timestamptz,
  constraint event_ticket_context_credential_pair_v4114_fk foreign key (integration_id,credential_version_id)
  ...
  constraint event_ticket_context_semantic_issue_v4114_unique unique (integration_id,credential_version_id,audience,operation_name,request_payload_hash,proof_nonce_hash),
  constraint event_ticket_context_consumed_receipt_v4114_unique unique (consumed_receipt_id),
  constraint event_ticket_context_status_v4114_check check (context_status in ('active','consumed','revoked','expired')),
  constraint event_ticket_context_contract_v4114_check check (audience ~ '^[a-z0-9][a-z0-9._:-]{2,79}$' and operation_name ~ '^[a-z0-9][a-z0-9._:-]{2,119}$'),
  constraint event_ticket_context_hashes_v4114_check check (
  ...
  constraint event_ticket_context_validity_v4114_check check (expires_at > issued_at),
  constraint event_ticket_context_terminal_v4114_check check (
...
create table public.event_ticket_click_attributions (
  ...
  constraint event_ticket_click_channel_semantic_v4114_fk
    ...
    on delete restrict,
  constraint event_ticket_click_semantic_v4114_unique
    unique (click_id, channel_id, canonical_event_id, integration_id),
  constraint event_ticket_click_redirect_v4114_unique unique (redirect_id),
  constraint event_ticket_click_hashes_v4114_check
    check (
...
create table public.event_ticket_purchase_signals (
  ...
  minimized_at timestamptz,
  constraint event_ticket_signal_credential_pair_v4114_fk
    foreign key (integration_id, credential_version_id)
    references public.event_ticket_trusted_integration_credential_versions (
      integration_id,
      credential_version_id
    ...
    on delete restrict,
  constraint event_ticket_signal_channel_semantic_v4114_fk
    foreign key (channel_id, canonical_event_id, integration_id)
    references public.event_ticket_commercial_channels (channel_id, canonical_event_id, integration_id)
    on delete restrict,
  constraint event_ticket_signal_click_semantic_v4114_fk
  ...
  constraint event_ticket_signal_status_v4114_check
    check (signal_status in ('recorded','superseded','tombstoned')),
  constraint event_ticket_signal_hashes_v4114_check
    check (
  ...
  constraint event_ticket_signal_trusted_evidence_v4114_check
    check (
  ...
  constraint event_ticket_signal_supersession_target_v4114_unique unique (signal_id,canonical_event_id,integration_id,transaction_family_id),
  constraint event_ticket_signal_supersession_v4114_fk foreign key (supersedes_signal_id,canonical_event_id,integration_id,transaction_family_id)
  ...
  constraint event_ticket_signal_click_requires_channel_v4114_check
    check (click_id is null or channel_id is not null)
);

create unique index event_ticket_signal_transaction_v4114_uq
...
create table public.partner_official_communications (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint partner_communications_status_v4114_check
    check (communication_status in ('draft','submitted','approved','published','paused','revoked','expired')),
  constraint partner_communications_audience_v4114_check
    check (audience_scope in ('all_active_clubbers','event_participants','partner_event_audience')),
  constraint partner_communications_hash_v4114_check
    check (body_hash ~ '^[0-9a-f]{64}$'),
  constraint partner_communications_validity_v4114_check
    check (ends_at is null or starts_at is null or ends_at > starts_at),
  constraint partner_communications_metadata_v4114_check
...
create table public.event_ticket_retention_runs (
  ...
  completed_at timestamptz,
  failed_at timestamptz,
  constraint event_ticket_retention_run_status_v4114_check check (run_status in ('running','completed','failed')),
  constraint event_ticket_retention_run_limit_v4114_check check (batch_limit between 1 and 5000),
  constraint event_ticket_retention_run_counts_v4114_check check (
  ...
  constraint event_ticket_retention_run_hashes_v4114_check check (
...
create table public.event_ticket_commercial_audit_log (
  ...
  minimized_at timestamptz,
  created_at timestamptz not null default now(),
  constraint event_ticket_audit_credential_pair_v4114_fk
    foreign key (integration_id, credential_version_id)
    references public.event_ticket_trusted_integration_credential_versions (
      integration_id,
      credential_version_id
    ...
    on delete restrict,
  constraint event_ticket_audit_identity_v4114_unique
    unique (target_type, target_id, idempotency_key),
  constraint event_ticket_audit_target_v4114_check
    check (target_type in (
...
create table public.event_ticket_backfill_rejections (
  ...
  minimized_at timestamptz,
  created_at timestamptz not null default now(),
  constraint event_ticket_backfill_identity_v4114_unique
    unique (reconciliation_run_key, source_table, source_primary_key),
  constraint event_ticket_backfill_source_v4114_check
    check (source_table in ('partner_ticket_requests','event_groups','event_ticket_intents')),
  constraint event_ticket_backfill_hash_v4114_check
...
values
  ('00000000-0000-4114-8000-000000000001'::uuid,'operation_receipt_br_template',1,'draft','operation_receipt','BR','operation_receipt','anonymize',730,repeat('0',64),public.mhidas_ticket_sha256_v1('operation_receipt_br_template_v1')),
  ('00000000-0000-4114-8000-000000000002'::uuid,'commercial_audit_br_template',1,'draft','commercial_audit','BR','commercial_audit','anonymize',1825,repeat('0',64),public.mhidas_ticket_sha256_v1('commercial_audit_br_template_v1')),
  ('00000000-0000-4114-8000-000000000003'::uuid,'purchase_signal_br_template',1,'draft','event_ticket_tracking','BR','purchase_signal','tombstone',730,repeat('0',64),public.mhidas_ticket_sha256_v1('purchase_signal_br_template_v1')),
  ('00000000-0000-4114-8000-000000000004'::uuid,'click_attribution_br_template',1,'draft','event_ticket_tracking','BR','click_attribution','anonymize',365,repeat('0',64),public.mhidas_ticket_sha256_v1('click_attribution_br_template_v1')),
  ('00000000-0000-4114-8000-000000000005'::uuid,'credential_context_br_template',1,'draft','security_audit','BR','credential_context','anonymize',90,repeat('0',64),public.mhidas_ticket_sha256_v1('credential_context_br_template_v1')),
  ('00000000-0000-4114-8000-000000000006'::uuid,'partner_communication_br_template',1,'draft','commercial_governance','BR','partner_communication','tombstone',730,repeat('0',64),public.mhidas_ticket_sha256_v1('partner_communication_br_template_v1')),
  ('00000000-0000-4114-8000-000000000007'::uuid,'backfill_rejection_br_template',1,'draft','commercial_governance','BR','backfill_rejection','anonymize',365,repeat('0',64),public.mhidas_ticket_sha256_v1('backfill_rejection_br_template_v1'));
...
values
  ('00000000-0000-4114-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','principal_id','nullify',null),
  ('00000000-0000-4114-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','integration_id','nullify',null),
  ('00000000-0000-4114-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','credential_version_id','nullify',null),
  ('00000000-0000-4114-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','principal_namespace','stable_hash','receipt.principal_namespace'),
  ('00000000-0000-4114-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','idempotency_key','stable_hash','receipt.idempotency'),
  ('00000000-0000-4114-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','target_id','stable_hash','receipt.target'),
  ('00000000-0000-4114-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','result_id','stable_hash','receipt.result'),
  ('00000000-0000-4114-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','correlation_id','stable_hash','receipt.correlation'),
  ('00000000-0000-4114-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','target_id','stable_hash','audit.target'),
  ('00000000-0000-4114-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','canonical_event_id','nullify',null),
  ('00000000-0000-4114-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','partner_id','nullify',null),
  ('00000000-0000-4114-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','integration_id','nullify',null),
  ('00000000-0000-4114-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','credential_version_id','nullify',null),
  ('00000000-0000-4114-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','actor_user_id','nullify',null),
  ('00000000-0000-4114-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','correlation_id','stable_hash','audit.correlation'),
  ('00000000-0000-4114-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','idempotency_key','stable_hash','audit.idempotency'),
  ('00000000-0000-4114-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','before_snapshot','nullify',null),
  ('00000000-0000-4114-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','after_snapshot','nullify',null),
  ('00000000-0000-4114-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','user_id','nullify',null),
  ('00000000-0000-4114-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','channel_id','nullify',null),
  ('00000000-0000-4114-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','click_id','nullify',null),
  ('00000000-0000-4114-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','credential_context_id','nullify',null),
  ('00000000-0000-4114-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','external_transaction_hash','stable_hash','signal.transaction'),
  ('00000000-0000-4114-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','evidence_metadata','replace_with_tombstone',null),
  ('00000000-0000-4114-8000-000000000004'::uuid,'click_attribution','event_ticket_click_attributions','user_id','nullify',null),
  ('00000000-0000-4114-8000-000000000004'::uuid,'click_attribution','event_ticket_click_attributions','visitor_hash','stable_hash','click.visitor'),
  ('00000000-0000-4114-8000-000000000004'::uuid,'click_attribution','event_ticket_click_attributions','campaign_hash','nullify',null),
  ('00000000-0000-4114-8000-000000000004'::uuid,'click_attribution','event_ticket_click_attributions','redirect_id','stable_hash','click.redirect'),
  ('00000000-0000-4114-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','context_token_hash','stable_hash','context.token'),
  ('00000000-0000-4114-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','request_payload_hash','stable_hash','context.request'),
  ('00000000-0000-4114-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','proof_nonce_hash','stable_hash','context.nonce'),
  ('00000000-0000-4114-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','proof_signature_hash','stable_hash','context.signature'),
  ('00000000-0000-4114-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','lifecycle_correlation_hash','stable_hash','context.correlation'),
  ('00000000-0000-4114-8000-000000000006'::uuid,'partner_communication','partner_official_communications','canonical_event_id','nullify',null),
  ('00000000-0000-4114-8000-000000000006'::uuid,'partner_communication','partner_official_communications','title','replace_with_tombstone',null),
  ('00000000-0000-4114-8000-000000000006'::uuid,'partner_communication','partner_official_communications','body_hash','stable_hash','communication.body'),
  ('00000000-0000-4114-8000-000000000006'::uuid,'partner_communication','partner_official_communications','approved_by_admin_user_id','nullify',null),
  ('00000000-0000-4114-8000-000000000006'::uuid,'partner_communication','partner_official_communications','published_by_admin_user_id','nullify',null),
  ('00000000-0000-4114-8000-000000000006'::uuid,'partner_communication','partner_official_communications','metadata','replace_with_tombstone',null),
  ('00000000-0000-4114-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','source_primary_key','stable_hash','backfill.source_key'),
  ('00000000-0000-4114-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','rejection_reason','replace_with_tombstone',null),
  ('00000000-0000-4114-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','canonical_event_id','nullify',null),
  ('00000000-0000-4114-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','source_payload_hash','stable_hash','backfill.payload'),
  ('00000000-0000-4114-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','reconciliation_run_key','stable_hash','backfill.run');
...
begin
  ...
  if public.mhidas_ticket_sha256_v1(p_issuer_service_key) <> v_authority.issuer_service_key_hash then
  ...
  if not public.mhidas_verify_detached_signature_v1(
...
begin
  ...
  if not found or v_context.context_status<>'active' or v_context.integration_id<>p_integration_id
    ...
    or v_context.request_payload_hash<>p_expected_request_payload_hash
...
begin
  ...
  if v_rule_count<>44 then raise exception 'RETENTION_MINIMIZATION_MATRIX_INCOMPLETE_V7'; end if;
...
begin
  ...
  begin
    ...
    set constraints event_ticket_integrations_current_credential_v4114_ct, event_ticket_credentials_current_invariant_v4114_ct immediate;
...
begin
  ...
  begin
    ...
    if not public.mhidas_ticket_url_proof_is_fresh_v8(v_channel.channel_id,v_channel.destination_url_hash,v_channel.url_validation_status,v_channel.destination_hostname,v_channel.url_validated_at,v_channel.url_validation_expires_at,v_channel.last_health_checked_at,v_channel.last_health_check_hash,v_channel.url_validator_version,v_channel.resolved_host_hash,v_channel.resolved_ip_set_hash,v_channel.redirect_chain_validation_hash,v_channel.url_proof_attestation_hash,v_channel.url_proof_manifest_hash,v_channel.url_proof_canonical_payload_hash,v_channel.url_proof_signature,v_channel.url_proof_signer_key_hash,v_channel.url_proof_signer_key_id,v_channel.url_proof_signature_algorithm,v_channel.url_validation_authority_id) then raise exception 'AUTHENTICATED_URL_PROOF_REQUIRED_V1'; end if;
  ...
  select p_status='validated'
    ...
    and exists (
      ...
      where a.url_validation_authority_id=p_authority_id and a.authority_status='active'
        ...
        and public.mhidas_verify_detached_signature_v1(a.signer_key_id,a.signature_algorithm,payload.canonical_payload,p_detached_signature)
    )
  from payload;
$mhidas_sql$;
...
create policy commercial_partners_admin_read_v4114 on public.commercial_partners for select to authenticated using (public.mhidas_is_useclubbers_admin_v1(auth.uid()));
create policy event_ticket_channels_admin_read_v4114 on public.event_ticket_commercial_channels for select to authenticated using (public.mhidas_is_useclubbers_admin_v1(auth.uid()));
create policy event_ticket_requests_owner_or_admin_read_v4114 on public.event_ticket_partnership_requests for select to authenticated using (submitted_by_user_id=auth.uid() or public.mhidas_is_useclubbers_admin_v1(auth.uid()));
create policy event_ticket_signals_admin_read_v4114 on public.event_ticket_purchase_signals for select to authenticated using (public.mhidas_is_useclubbers_admin_v1(auth.uid()));
create policy event_ticket_audit_admin_read_v4114 on public.event_ticket_commercial_audit_log for select to authenticated using (public.mhidas_is_useclubbers_admin_v1(auth.uid()));
...
-- =============================================================================
-- 13. TENTH-CORRECTED STRUCTURAL SELF-CHECK
-- =============================================================================

do $mhidas_v4114_self_check$
declare v_count integer;
begin
  select count(*) into v_count from public.event_ticket_retention_policy_versions where policy_key like '%_br_template';
  if v_count<>7 then raise exception 'V4_8_114_RETENTION_POLICY_TEMPLATE_COUNT_INVALID'; end if;
  select count(*) into v_count from public.event_ticket_retention_minimization_rules where rule_status='active';
  if v_count<>44 then raise exception 'V4_8_114_MINIMIZATION_RULE_COUNT_INVALID'; end if;
  select count(*) into v_count from public.event_ticket_operation_result_contracts where contract_status='active';
  if v_count<>9 then raise exception 'V4_8_114_RESULT_CONTRACT_COUNT_INVALID'; end if;
  if (select provolatile from pg_proc where oid='public.mhidas_ticket_resolve_active_retention_policy_v2(text,text,text)'::regprocedure)<>'v' then raise exception 'V4_8_114_RETENTION_RESOLVER_NOT_VOLATILE'; end if;
  if not exists (select 1 from pg_trigger where tgname='event_ticket_integrations_current_credential_v4114_ct' and tgdeferrable) then raise exception 'V4_8_114_INTEGRATION_CONSTRAINT_TRIGGER_MISSING'; end if;
  if exists (select 1 from public.event_ticket_trusted_integrations i where i.integration_status='active' and (i.current_credential_version_id is null or i.issuer_service_key_hash is null)) then raise exception 'V4_8_114_ACTIVE_INTEGRATION_AUTHORITY_DRIFT'; end if;
  if exists (select integration_id from public.event_ticket_trusted_integration_credential_versions where credential_status='active' group by integration_id having count(*)>1) then raise exception 'V4_8_114_MULTIPLE_ACTIVE_CREDENTIALS'; end if;
  if exists (select 1 from public.event_ticket_commercial_channels c where c.channel_status='active' and (c.authorization_receipt_id is null or c.financial_terms_hash is null or c.lifecycle_snapshot_hash is null or c.authorized_scope_snapshot_hash is null or c.url_validation_authority_id is null or c.url_proof_signature is null)) then raise exception 'V4_8_114_ACTIVE_CHANNEL_AUTHORITY_DRIFT'; end if;
  if exists (select 1 from public.event_ticket_purchase_signals s where s.external_transaction_hash is not null and s.transaction_family_id is null) then raise exception 'V4_8_114_SIGNAL_FAMILY_ID_DRIFT'; end if;
  if exists (select 1 from public.event_ticket_operation_receipts r where r.receipt_status='completed' and not exists (select 1 from public.event_ticket_operation_result_contracts c where c.operation_name=r.operation_name and c.result_status=r.result_status and c.contract_status='active' and r.result_version>=c.minimum_result_version and (c.maximum_result_version is null or r.result_version<=c.maximum_result_version))) then raise exception 'V4_8_114_RECEIPT_RESULT_CONTRACT_DRIFT'; end if;
  if exists (
  ...
  ) then raise exception 'V4_8_114_RETIRED_POLICY_SUCCESSOR_DRIFT'; end if;
  if exists (
    select 1
    from unnest(array[
      'commercial_partners','commercial_partner_representatives','event_ticket_trusted_integrations',
      'event_ticket_trusted_integration_credential_versions','event_ticket_trusted_integration_channels',
      'event_ticket_partnership_requests','event_ticket_retention_policy_versions',
  ...
  ) then raise exception 'V4_8_114_SERVICE_ROLE_EFFECTIVE_DML_GRANT_DRIFT'; end if;
  ...
  if v_count<>5 then raise exception 'V4_8_114_RLS_SELECT_GRANT_COUNT_INVALID'; end if;
  if position('destination_url_ciphertext' in pg_get_function_result('public.mhidas_resolve_public_event_ticket_action_v8(uuid)'::regprocedure))>0 then raise exception 'V4_8_114_PUBLIC_RESOLVER_EXPOSES_CIPHERTEXT'; end if;
end;
$mhidas_v4114_self_check$;

-- =============================================================================
-- 14. EXPLICIT PROTECTED-DRAFT DECISION
-- =============================================================================

-- parsed_postgres_statement_count=121
-- draft_decision=tenth_corrected_adjusted_draft_ready_for_eleventh_structural_review
...
-- v4_8_112_ninth_corrected_sql_changed=False
