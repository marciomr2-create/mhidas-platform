-- docs/sql/EVENT_TICKET_COMMERCIAL_MIGRATION_ELEVENTH_CORRECTED_ADJUSTED_DRAFT.sql
-- Version: v4.8.116-event-ticket-commercial-migration-eleventh-corrected-adjusted-draft-safe
-- Base: v4.8.115-event-ticket-commercial-migration-tenth-corrected-adjusted-draft-structural-review-safe
--
...
do $mhidas_guard$
begin
  raise exception 'MHIDAS_PROTECTED_DRAFT_V4_8_116';
end;
$mhidas_guard$;
...
begin
  ...
  foreach v_name in array array[
    ...
    'event_ticket_operation_result_contracts'
  ] loop
  ...
  for v_item in
    ...
    where schemaname = 'public' and policyname = any(array[
      'commercial_partners_admin_read_v4116',
      'event_ticket_channels_admin_read_v4116',
      'event_ticket_requests_owner_or_admin_read_v4116',
      'event_ticket_signals_admin_read_v4116',
      'event_ticket_audit_admin_read_v4116'
    ])
  loop
  ...
  foreach v_name in array array[
    'event_ticket_credentials_one_active_v4116_uq',
    'event_ticket_retention_policy_active_dimension_v4116_uq',
    'event_ticket_channels_one_active_v4116_uq',
    'event_ticket_receipts_semantic_v4116_uq',
    'event_ticket_signal_transaction_v4116_uq'
  ] loop
  ...
  if cardinality(v_missing) > 0 or cardinality(v_drift) > 0 then
    raise exception 'V4_8_116_BASE_PREFLIGHT_FAILED missing=[%] drift=[%]', array_to_string(v_missing, ','), array_to_string(v_drift, ',');
  end if;
end;
$mhidas_preflight$;
...
create function public.mhidas_ticket_lock_current_credential_v3(
...
begin
  return query
  select p.partner_id, i.principal_namespace, c.verifier_key_hash, c.credential_key_id, c.signature_algorithm, i.issuer_service_key_hash
...
create table public.commercial_partner_representatives (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint commercial_partner_representatives_identity_v4116_unique
    unique (partner_id, user_id),
  constraint commercial_partner_representatives_role_v4116_check
    check (representative_role in ('owner','commercial','marketing','operations','legal')),
  constraint commercial_partner_representatives_status_v4116_check
    check (representation_status in ('pending','active','suspended','revoked')),
  constraint commercial_partner_representatives_validity_v4116_check
    check (valid_until is null or valid_from is null or valid_until > valid_from),
  constraint commercial_partner_representatives_metadata_v4116_check
...
create table public.event_ticket_trusted_integrations (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_integrations_key_v4116_check
    check (integration_key ~ '^[a-z0-9][a-z0-9_.-]{2,79}$'),
  constraint event_ticket_integrations_namespace_v4116_check
    check (principal_namespace ~ '^[a-z0-9][a-z0-9_.-]{2,79}$'),
  constraint event_ticket_integrations_status_v4116_check
    check (integration_status in ('pending','active','suspended','retired')),
  constraint event_ticket_integrations_partner_v4116_check
  ...
  constraint event_ticket_integrations_metadata_v4116_check
...
create table public.event_ticket_trusted_integration_credential_versions (
  ...
  created_at timestamptz not null default now(),
  constraint event_ticket_credentials_pair_v4116_unique
    unique (integration_id, credential_version_id),
  constraint event_ticket_credentials_version_v4116_unique
    unique (integration_id, credential_version),
  constraint event_ticket_credentials_status_v4116_check
    check (credential_status in ('pending','active','rotating','revoked','expired')),
  constraint event_ticket_credentials_hashes_v4116_check
    check (
  ...
  constraint event_ticket_credentials_validity_v4116_check
    check (valid_until is null or valid_until > valid_from)
);

create unique index event_ticket_credentials_one_active_v4116_uq
...
alter table public.event_ticket_trusted_integrations
  add constraint event_ticket_integrations_current_credential_v4116_fk
...
create constraint trigger event_ticket_integrations_current_credential_v4116_ct
...
create constraint trigger event_ticket_credentials_current_invariant_v4116_ct
...
create table public.event_ticket_trusted_integration_channels (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_integration_channels_identity_v4116_unique
    unique (integration_id, canonical_event_id, channel_scope),
  constraint event_ticket_integration_channels_authority_v4116_unique
    unique (integration_channel_id, integration_id, canonical_event_id),
  constraint event_ticket_integration_channels_authority_scope_v4116_unique
    unique (integration_channel_id, integration_id, canonical_event_id, channel_scope),
  constraint event_ticket_integration_channels_scope_v4116_check
    check (channel_scope in ('purchase_signal','url_validation','conversion_confirmation')),
  constraint event_ticket_integration_channels_status_v4116_check
    check (scope_status in ('pending','active','suspended','revoked')),
  constraint event_ticket_integration_channels_validity_v4116_check
    check (valid_until is null or valid_from is null or valid_until > valid_from)
);
...
create table public.event_ticket_partnership_requests (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_requests_submission_v4116_unique
    unique (partner_id, client_submission_key),
  constraint event_ticket_requests_authorization_v4116_unique
    unique (request_id, canonical_event_id, partner_id),
  constraint event_ticket_requests_type_v4116_check
  ...
  constraint event_ticket_requests_status_v4116_check
    check (request_status in ('pending','needs_info','approved','rejected','withdrawn')),
  constraint event_ticket_requests_hashes_v4116_check
    check (
  ...
  constraint event_ticket_requests_metadata_v4116_check
...
create table public.event_ticket_retention_policy_bootstrap_receipts (
  ...
  created_at timestamptz not null default now(),
  constraint event_ticket_retention_bootstrap_status_v4116_check
    check (bootstrap_status = 'completed'),
  constraint event_ticket_retention_bootstrap_count_v4116_check
    check (policy_count = 7),
  constraint event_ticket_retention_bootstrap_hashes_v4116_check
...
create table public.event_ticket_retention_policy_versions (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_retention_policy_version_v4116_unique
    unique (policy_key, policy_version),
  constraint event_ticket_retention_policy_status_v4116_check
    check (policy_status in ('draft','approved','active','retired')),
  constraint event_ticket_retention_policy_dimensions_v4116_check
    check (
      policy_purpose in (
  ...
  constraint event_ticket_retention_policy_action_v4116_check
    check (retention_action in ('delete','anonymize','tombstone')),
  constraint event_ticket_retention_policy_days_v4116_check
    check (retention_days between 1 and 3650),
  constraint event_ticket_retention_policy_hashes_v4116_check
    check (
      legal_basis_reference_hash ~ '^[0-9a-f]{64}$'
      and policy_manifest_hash ~ '^[0-9a-f]{64}$'
      and executor_contract_hash ~ '^[0-9a-f]{64}$'
    ),
  constraint event_ticket_retention_policy_approval_v4116_check
    check (
      policy_status = 'draft'
      or (approved_at is not null and (approved_by_admin_user_id is not null or bootstrap_receipt_id is not null))
    ),
  constraint event_ticket_retention_policy_activation_v4116_check
    check (policy_status <> 'active' or activated_at is not null),
  constraint event_ticket_retention_policy_retirement_v4116_check
    check (
...
alter table public.event_ticket_retention_policy_versions
  add constraint event_ticket_retention_policy_successor_v4116_fk
...
create unique index event_ticket_retention_policy_active_dimension_v4116_uq
...
create table public.event_ticket_retention_minimization_rules (
  ...
  created_at timestamptz not null default now(),
  constraint event_ticket_minimization_rule_identity_v4116_unique
    unique (retention_policy_version_id, evidence_class, source_table, source_field),
  constraint event_ticket_minimization_rule_action_v4116_check
    check (minimization_action in ('delete','nullify','stable_hash','retain','replace_with_tombstone')),
  constraint event_ticket_minimization_rule_hash_v4116_check
    check (
      minimization_action <> 'stable_hash'
      or stable_hash_namespace ~ '^[a-z0-9][a-z0-9._-]{2,80}$'
    ),
  constraint event_ticket_minimization_rule_status_v4116_check
...
create table public.event_ticket_url_validation_authorities (
  ...
  approved_at timestamptz,
  created_at timestamptz not null default now(),
  constraint event_ticket_url_authority_status_v4116_check check (authority_status in ('pending','active','suspended','retired')),
  constraint event_ticket_url_authority_hash_v4116_check check (
    signer_key_hash ~ '^[0-9a-f]{64}$'
    and signer_key_id ~ '^[a-zA-Z0-9][a-zA-Z0-9._:/-]{2,199}$'
    and signature_algorithm in ('ed25519','ecdsa-p256-sha256','rsa-pss-sha256')
  ),
  constraint event_ticket_url_authority_validity_v4116_check check (valid_until is null or valid_until > valid_from)
);
...
create table public.event_ticket_official_source_attestations (
  ...
  created_at timestamptz not null default now(),
  constraint event_ticket_official_source_identity_v4116_unique unique (canonical_event_id, source_url),
  constraint event_ticket_official_source_hashes_v4116_check check (
  ...
  constraint event_ticket_official_source_signature_v4116_check check (
  ...
  constraint event_ticket_official_source_host_v4116_check check (
  ...
  constraint event_ticket_official_source_policy_v4116_check check (commercial_policy_status in ('official_noncommercial','commercial_authorized')),
  constraint event_ticket_official_source_freshness_v4116_check check (expires_at > validated_at and expires_at <= validated_at + interval '30 days')
);
...
create table public.event_ticket_commercial_channels (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint event_ticket_channels_semantic_v4116_unique unique (channel_id, canonical_event_id, integration_id),
  constraint event_ticket_channels_request_v4116_fk foreign key (request_id,canonical_event_id,partner_id)
    references public.event_ticket_partnership_requests(request_id,canonical_event_id,partner_id) on delete restrict,
  constraint event_ticket_channels_scope_v4116_fk foreign key (integration_channel_id,integration_id,canonical_event_id)
  ...
  constraint event_ticket_channels_status_v4116_check check (channel_status in ('draft','authorized','active','paused','expired','revoked')),
  constraint event_ticket_channels_kind_v4116_check check (channel_kind in ('official_reference','affiliate','discount','presale','fixed_media','hybrid')),
  constraint event_ticket_channels_tracking_v4116_check check (tracking_method in ('none','redirect','coupon','postback','webhook','partner_api')),
  constraint event_ticket_channels_financial_v4116_check check (
  ...
  constraint event_ticket_channels_hashes_v4116_check check (
  ...
  constraint event_ticket_channels_active_authority_v4116_check check (
  ...
  constraint event_ticket_channels_metadata_v4116_check check (public.mhidas_ticket_json_matches_contract_v2('channel_metadata', metadata))
);

create unique index event_ticket_channels_one_active_v4116_uq
...
create table public.event_ticket_operation_result_contracts (
  ...
  constraint event_ticket_result_contract_name_v4116_check check (operation_name ~ '^[a-z0-9][a-z0-9._:-]{2,119}$'),
  constraint event_ticket_result_contract_status_v4116_check check (
    result_status ~ '^[a-z0-9][a-z0-9._:-]{2,79}$'
    and result_status not in ('pending','failed')
    and contract_status in ('active','retired')
  ),
  constraint event_ticket_result_contract_version_v4116_check check (
...
create table public.event_ticket_operation_receipts (
  ...
  constraint event_ticket_receipts_credential_pair_v4116_fk
    ...
    on delete restrict,
  constraint event_ticket_receipts_scope_v4116_check
  ...
  constraint event_ticket_receipts_principal_v4116_check
    check (
  ...
  constraint event_ticket_receipts_target_v4116_check
  ...
  constraint event_ticket_receipts_hashes_v4116_check
    check (
  ...
  constraint event_ticket_receipts_state_v4116_check
    check (
...
create unique index event_ticket_receipts_semantic_v4116_uq
...
alter table public.event_ticket_commercial_channels
  add constraint event_ticket_channels_authorization_receipt_v4116_fk
...
create table public.event_ticket_verified_credential_contexts (
  ...
  minimized_at timestamptz,
  constraint event_ticket_context_credential_pair_v4116_fk foreign key (integration_id,credential_version_id)
  ...
  constraint event_ticket_context_semantic_issue_v4116_unique unique (integration_id,credential_version_id,audience,operation_name,request_payload_hash,proof_nonce_hash),
  constraint event_ticket_context_consumed_receipt_v4116_unique unique (consumed_receipt_id),
  constraint event_ticket_context_status_v4116_check check (context_status in ('active','consumed','revoked','expired')),
  constraint event_ticket_context_contract_v4116_check check (audience ~ '^[a-z0-9][a-z0-9._:-]{2,79}$' and operation_name ~ '^[a-z0-9][a-z0-9._:-]{2,119}$'),
  constraint event_ticket_context_hashes_v4116_check check (
  ...
  constraint event_ticket_context_validity_v4116_check check (expires_at > issued_at),
  constraint event_ticket_context_terminal_v4116_check check (
...
create table public.event_ticket_click_attributions (
  ...
  constraint event_ticket_click_channel_semantic_v4116_fk
    ...
    on delete restrict,
  constraint event_ticket_click_semantic_v4116_unique
    unique (click_id, channel_id, canonical_event_id, integration_id),
  constraint event_ticket_click_redirect_v4116_unique unique (redirect_id),
  constraint event_ticket_click_hashes_v4116_check
    check (
...
create table public.event_ticket_purchase_signals (
  ...
  minimized_at timestamptz,
  constraint event_ticket_signal_credential_pair_v4116_fk
    foreign key (integration_id, credential_version_id)
    references public.event_ticket_trusted_integration_credential_versions (
      integration_id,
      credential_version_id
    ...
    on delete restrict,
  constraint event_ticket_signal_channel_semantic_v4116_fk
    foreign key (channel_id, canonical_event_id, integration_id)
    references public.event_ticket_commercial_channels (channel_id, canonical_event_id, integration_id)
    on delete restrict,
  constraint event_ticket_signal_click_semantic_v4116_fk
  ...
  constraint event_ticket_signal_status_v4116_check
    check (signal_status in ('recorded','superseded','tombstoned')),
  constraint event_ticket_signal_hashes_v4116_check
    check (
  ...
  constraint event_ticket_signal_trusted_evidence_v4116_check
    check (
  ...
  constraint event_ticket_signal_supersession_target_v4116_unique unique (signal_id,canonical_event_id,integration_id,transaction_family_id),
  constraint event_ticket_signal_supersession_v4116_fk foreign key (supersedes_signal_id,canonical_event_id,integration_id,transaction_family_id)
  ...
  constraint event_ticket_signal_click_requires_channel_v4116_check
    check (click_id is null or channel_id is not null)
);

create unique index event_ticket_signal_transaction_v4116_uq
...
create table public.partner_official_communications (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint partner_communications_status_v4116_check
    check (communication_status in ('draft','submitted','approved','published','paused','revoked','expired')),
  constraint partner_communications_audience_v4116_check
    check (audience_scope in ('all_active_clubbers','event_participants','partner_event_audience')),
  constraint partner_communications_hash_v4116_check
    check (body_hash ~ '^[0-9a-f]{64}$'),
  constraint partner_communications_validity_v4116_check
    check (ends_at is null or starts_at is null or ends_at > starts_at),
  constraint partner_communications_metadata_v4116_check
...
create table public.event_ticket_retention_runs (
  ...
  completed_at timestamptz,
  failed_at timestamptz,
  constraint event_ticket_retention_run_status_v4116_check check (run_status in ('running','completed','failed')),
  constraint event_ticket_retention_run_limit_v4116_check check (batch_limit between 1 and 5000),
  constraint event_ticket_retention_run_counts_v4116_check check (
  ...
  constraint event_ticket_retention_run_hashes_v4116_check check (
...
create table public.event_ticket_commercial_audit_log (
  ...
  minimized_at timestamptz,
  created_at timestamptz not null default now(),
  constraint event_ticket_audit_credential_pair_v4116_fk
    foreign key (integration_id, credential_version_id)
    references public.event_ticket_trusted_integration_credential_versions (
      integration_id,
      credential_version_id
    ...
    on delete restrict,
  constraint event_ticket_audit_identity_v4116_unique
    unique (target_type, target_id, idempotency_key),
  constraint event_ticket_audit_target_v4116_check
    check (target_type in (
...
create table public.event_ticket_backfill_rejections (
  ...
  minimized_at timestamptz,
  created_at timestamptz not null default now(),
  constraint event_ticket_backfill_identity_v4116_unique
    unique (reconciliation_run_key, source_table, source_primary_key),
  constraint event_ticket_backfill_source_v4116_check
    check (source_table in ('partner_ticket_requests','event_groups','event_ticket_intents')),
  constraint event_ticket_backfill_hash_v4116_check
...
insert into public.event_ticket_retention_policy_bootstrap_receipts (
  ...
  '00000000-0000-4116-8000-000000000000'::uuid,
...
values
  ('00000000-0000-4116-8000-000000000001'::uuid,'operation_receipt_br_template',1,'active','operation_receipt','BR','operation_receipt','anonymize',730,repeat('0',64),public.mhidas_ticket_sha256_v1('operation_receipt_br_template_v1'),now(),now(),'00000000-0000-4116-8000-000000000000'::uuid,public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:bootstrap-pending')),
  ('00000000-0000-4116-8000-000000000002'::uuid,'commercial_audit_br_template',1,'active','commercial_audit','BR','commercial_audit','anonymize',1825,repeat('0',64),public.mhidas_ticket_sha256_v1('commercial_audit_br_template_v1'),now(),now(),'00000000-0000-4116-8000-000000000000'::uuid,public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:bootstrap-pending')),
  ('00000000-0000-4116-8000-000000000003'::uuid,'purchase_signal_br_template',1,'active','event_ticket_tracking','BR','purchase_signal','tombstone',730,repeat('0',64),public.mhidas_ticket_sha256_v1('purchase_signal_br_template_v1'),now(),now(),'00000000-0000-4116-8000-000000000000'::uuid,public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:bootstrap-pending')),
  ('00000000-0000-4116-8000-000000000004'::uuid,'click_attribution_br_template',1,'active','event_ticket_tracking','BR','click_attribution','anonymize',365,repeat('0',64),public.mhidas_ticket_sha256_v1('click_attribution_br_template_v1'),now(),now(),'00000000-0000-4116-8000-000000000000'::uuid,public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:bootstrap-pending')),
  ('00000000-0000-4116-8000-000000000005'::uuid,'credential_context_br_template',1,'active','security_audit','BR','credential_context','anonymize',90,repeat('0',64),public.mhidas_ticket_sha256_v1('credential_context_br_template_v1'),now(),now(),'00000000-0000-4116-8000-000000000000'::uuid,public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:bootstrap-pending')),
  ('00000000-0000-4116-8000-000000000006'::uuid,'partner_communication_br_template',1,'active','commercial_governance','BR','partner_communication','tombstone',730,repeat('0',64),public.mhidas_ticket_sha256_v1('partner_communication_br_template_v1'),now(),now(),'00000000-0000-4116-8000-000000000000'::uuid,public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:bootstrap-pending')),
  ('00000000-0000-4116-8000-000000000007'::uuid,'backfill_rejection_br_template',1,'active','commercial_governance','BR','backfill_rejection','anonymize',365,repeat('0',64),public.mhidas_ticket_sha256_v1('backfill_rejection_br_template_v1'),now(),now(),'00000000-0000-4116-8000-000000000000'::uuid,public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:bootstrap-pending'));
...
values
  ('00000000-0000-4116-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','principal_id','nullify',null),
  ('00000000-0000-4116-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','integration_id','nullify',null),
  ('00000000-0000-4116-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','credential_version_id','nullify',null),
  ('00000000-0000-4116-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','principal_namespace','stable_hash','receipt.principal_namespace'),
  ('00000000-0000-4116-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','idempotency_key','stable_hash','receipt.idempotency'),
  ('00000000-0000-4116-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','target_id','stable_hash','receipt.target'),
  ('00000000-0000-4116-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','result_id','stable_hash','receipt.result'),
  ('00000000-0000-4116-8000-000000000001'::uuid,'operation_receipt','event_ticket_operation_receipts','correlation_id','stable_hash','receipt.correlation'),
  ('00000000-0000-4116-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','target_id','stable_hash','audit.target'),
  ('00000000-0000-4116-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','canonical_event_id','nullify',null),
  ('00000000-0000-4116-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','partner_id','nullify',null),
  ('00000000-0000-4116-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','integration_id','nullify',null),
  ('00000000-0000-4116-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','credential_version_id','nullify',null),
  ('00000000-0000-4116-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','actor_user_id','nullify',null),
  ('00000000-0000-4116-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','correlation_id','stable_hash','audit.correlation'),
  ('00000000-0000-4116-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','idempotency_key','stable_hash','audit.idempotency'),
  ('00000000-0000-4116-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','before_snapshot','nullify',null),
  ('00000000-0000-4116-8000-000000000002'::uuid,'commercial_audit','event_ticket_commercial_audit_log','after_snapshot','nullify',null),
  ('00000000-0000-4116-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','user_id','nullify',null),
  ('00000000-0000-4116-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','channel_id','nullify',null),
  ('00000000-0000-4116-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','click_id','nullify',null),
  ('00000000-0000-4116-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','credential_context_id','nullify',null),
  ('00000000-0000-4116-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','external_transaction_hash','stable_hash','signal.transaction'),
  ('00000000-0000-4116-8000-000000000003'::uuid,'purchase_signal','event_ticket_purchase_signals','evidence_metadata','replace_with_tombstone',null),
  ('00000000-0000-4116-8000-000000000004'::uuid,'click_attribution','event_ticket_click_attributions','user_id','nullify',null),
  ('00000000-0000-4116-8000-000000000004'::uuid,'click_attribution','event_ticket_click_attributions','visitor_hash','stable_hash','click.visitor'),
  ('00000000-0000-4116-8000-000000000004'::uuid,'click_attribution','event_ticket_click_attributions','campaign_hash','nullify',null),
  ('00000000-0000-4116-8000-000000000004'::uuid,'click_attribution','event_ticket_click_attributions','redirect_id','stable_hash','click.redirect'),
  ('00000000-0000-4116-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','context_token_hash','stable_hash','context.token'),
  ('00000000-0000-4116-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','request_payload_hash','stable_hash','context.request'),
  ('00000000-0000-4116-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','proof_nonce_hash','stable_hash','context.nonce'),
  ('00000000-0000-4116-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','proof_signature_hash','stable_hash','context.signature'),
  ('00000000-0000-4116-8000-000000000005'::uuid,'credential_context','event_ticket_verified_credential_contexts','lifecycle_correlation_hash','stable_hash','context.correlation'),
  ('00000000-0000-4116-8000-000000000006'::uuid,'partner_communication','partner_official_communications','canonical_event_id','nullify',null),
  ('00000000-0000-4116-8000-000000000006'::uuid,'partner_communication','partner_official_communications','title','replace_with_tombstone',null),
  ('00000000-0000-4116-8000-000000000006'::uuid,'partner_communication','partner_official_communications','body_hash','stable_hash','communication.body'),
  ('00000000-0000-4116-8000-000000000006'::uuid,'partner_communication','partner_official_communications','approved_by_admin_user_id','nullify',null),
  ('00000000-0000-4116-8000-000000000006'::uuid,'partner_communication','partner_official_communications','published_by_admin_user_id','nullify',null),
  ('00000000-0000-4116-8000-000000000006'::uuid,'partner_communication','partner_official_communications','metadata','replace_with_tombstone',null),
  ('00000000-0000-4116-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','source_primary_key','stable_hash','backfill.source_key'),
  ('00000000-0000-4116-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','rejection_reason','replace_with_tombstone',null),
  ('00000000-0000-4116-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','canonical_event_id','nullify',null),
  ('00000000-0000-4116-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','source_payload_hash','stable_hash','backfill.payload'),
  ('00000000-0000-4116-8000-000000000007'::uuid,'backfill_rejection','event_ticket_backfill_rejections','reconciliation_run_key','stable_hash','backfill.run');
...
values
  ('issue_verified_credential_context_v6','issued',1,1),
  ('record_purchase_signal_v11','recorded',1,1),
  ('run_governance_retention_batch_v7','completed',1,1),
  ...
  ('authorize_activate_commercial_channel_v2','active',1,null);

update public.event_ticket_retention_policy_versions p
set policy_manifest_hash = x.rule_manifest_hash,
    executor_contract_hash = public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:' || x.rule_manifest_hash),
    updated_at = now()
from (
  select r.retention_policy_version_id,
    public.mhidas_ticket_sha256_v1(string_agg(
      r.evidence_class || ':' || r.source_table || ':' || r.source_field || ':' || r.minimization_action || ':' || coalesce(r.stable_hash_namespace,''),
      '|' order by r.evidence_class,r.source_table,r.source_field,r.minimization_action,coalesce(r.stable_hash_namespace,'')
    )) as rule_manifest_hash
  from public.event_ticket_retention_minimization_rules r
  where r.rule_status='active'
  group by r.retention_policy_version_id
) x
where x.retention_policy_version_id=p.retention_policy_version_id;

create function public.mhidas_ticket_assert_retention_execution_contract_v1(
  p_policy_id uuid,
  p_expected_evidence_class text,
...
begin
  ...
  where retention_policy_version_id=p_policy_id;
  if not found or v_policy.policy_status not in ('approved','active','retired')
    or v_policy.evidence_class is distinct from p_expected_evidence_class then
    raise exception 'RETENTION_EXECUTION_POLICY_AUTHORITY_DENIED_V1';
  end if;
  select count(*),public.mhidas_ticket_sha256_v1(string_agg(
      r.evidence_class || ':' || r.source_table || ':' || r.source_field || ':' || r.minimization_action || ':' || coalesce(r.stable_hash_namespace,''),
      '|' order by r.evidence_class,r.source_table,r.source_field,r.minimization_action,coalesce(r.stable_hash_namespace,'')
    ))
  into v_count,v_manifest
  from public.event_ticket_retention_minimization_rules r
  where r.retention_policy_version_id=p_policy_id and r.rule_status='active';
  if v_count is distinct from p_expected_rule_count
    or v_manifest is distinct from v_policy.policy_manifest_hash
    or v_policy.executor_contract_hash is distinct from public.mhidas_ticket_sha256_v1('mhidas.retention-executor.v8:'||v_manifest) then
    raise exception 'RETENTION_EXECUTION_CONTRACT_DRIFT_V1';
  end if;
end;
$mhidas_plpgsql$;
...
create function public.mhidas_ticket_reserve_operation_receipt_v9(
...
create function public.mhidas_ticket_issue_verified_credential_context_v6(
  p_integration_id uuid,
  p_credential_version_id uuid,
...
declare
  ...
  v_failure_hash text;
begin
  select * into v_authority from public.mhidas_ticket_lock_current_credential_v3(p_integration_id,p_credential_version_id);
  ...
  if public.mhidas_verify_detached_signature_v1(
  ...
  v_receipt := public.mhidas_ticket_reserve_operation_receipt_v9(
    'credential_context_mutation','trusted_ticketing_integration',null,v_authority.principal_namespace,
    p_integration_id,p_credential_version_id,'issue_verified_credential_context_v6',p_idempotency_key,
  ...
  begin
    insert into public.event_ticket_verified_credential_contexts (
      ...
      verifier_key_hash_snapshot,issuer_service_key_hash_snapshot,issuance_idempotency_key,issuance_receipt_id,
      ...
      v_authority.verifier_key_hash,v_authority.issuer_service_key_hash,p_idempotency_key,v_receipt.receipt_id,
...
create function public.mhidas_record_event_ticket_purchase_signal_v11(
...
begin
  ...
  v_receipt:=public.mhidas_ticket_reserve_operation_receipt_v9('signal_insert','trusted_ticketing_integration',null,v_authority.principal_namespace,p_integration_id,p_credential_version_id,'record_purchase_signal_v11',p_idempotency_key,'purchase_signal',v_signal_id,null,v_request_hash,p_correlation_id,v_receipt_policy);
  if v_receipt.receipt_status='completed' then
  ...
  begin
    perform public.mhidas_ticket_consume_verified_credential_context_v7(p_credential_context_id,p_context_token,'purchase_signal','record_purchase_signal_v11',v_request_hash,v_receipt.receipt_id,v_signal_id,p_integration_id,p_credential_version_id);
...
create function public.mhidas_run_event_ticket_governance_retention_batch_v7(
...
begin
  if p_batch_limit not between 1 and 5000 then raise exception 'RETENTION_BATCH_LIMIT_INVALID_V7'; end if;
  v_receipt_policy:=public.mhidas_ticket_resolve_active_retention_policy_v2('operation_receipt','BR','operation_receipt');
  ...
  perform public.mhidas_ticket_assert_retention_execution_contract_v1(v_receipt_policy,'operation_receipt',8);
  perform public.mhidas_ticket_assert_retention_execution_contract_v1(v_audit_policy,'commercial_audit',10);
  perform public.mhidas_ticket_assert_retention_execution_contract_v1(v_signal_policy,'purchase_signal',6);
  perform public.mhidas_ticket_assert_retention_execution_contract_v1(v_click_policy,'click_attribution',4);
  perform public.mhidas_ticket_assert_retention_execution_contract_v1(v_context_policy,'credential_context',5);
  perform public.mhidas_ticket_assert_retention_execution_contract_v1(v_communication_policy,'partner_communication',5);
  perform public.mhidas_ticket_assert_retention_execution_contract_v1(v_backfill_policy,'backfill_rejection',6);
  ...
  v_receipt:=public.mhidas_ticket_reserve_operation_receipt_v9('retention_run','service_role',null,'service_role:retention',null,null,'run_governance_retention_batch_v7',p_idempotency_key,'retention_run',v_run_id,null,v_request_hash,p_correlation_id,v_receipt_policy);
  ...
  begin
    ...
    with due_families as (
      select s.transaction_family_id
      ...
      where p.policy_status in ('active','retired') and p.policy_purpose='event_ticket_tracking' and p.jurisdiction_code='BR' and p.evidence_class='purchase_signal'
        and s.created_at<now()-make_interval(days=>p.retention_days) and s.minimized_at is null and s.transaction_family_id is not null
      group by s.transaction_family_id
      order by min(s.created_at)
      limit p_batch_limit
    ), locked_families as (
      select f.transaction_family_id
      from due_families f
      where pg_try_advisory_xact_lock(hashtextextended(f.transaction_family_id::text, 0))
    ), c as (
      select s.signal_id
      from public.event_ticket_purchase_signals s
      join locked_families f on f.transaction_family_id=s.transaction_family_id
...
begin
  ...
  v_receipt:=public.mhidas_ticket_reserve_operation_receipt_v9('credential_rotation','user',auth.uid(),'auth:user',null,null,'rotate_integration_credential_v2',p_idempotency_key,'trusted_integration',p_integration_id,p_expected_lock_version,v_request_hash,p_correlation_id,v_receipt_policy);
  ...
  begin
    ...
    set constraints event_ticket_integrations_current_credential_v4116_ct, event_ticket_credentials_current_invariant_v4116_ct immediate;
...
begin
  ...
  begin
    ...
    perform public.mhidas_ticket_assert_retention_execution_contract_v1(
...
create function public.mhidas_admin_authorize_activate_commercial_channel_v2(
  p_channel_id uuid,
  p_expected_lock_version integer,
...
begin
  ...
  v_receipt_policy:=public.mhidas_ticket_resolve_active_retention_policy_v2('operation_receipt','BR','operation_receipt');
  v_audit_policy:=public.mhidas_ticket_resolve_active_retention_policy_v2('commercial_audit','BR','commercial_audit');
  v_request_hash:=public.mhidas_ticket_sha256_v1(public.mhidas_ticket_canonical_payload_v1('mhidas.channel-activation.v2',jsonb_build_object('channel_id',p_channel_id,'expected_lock_version',p_expected_lock_version,'financial_terms_hash',p_financial_terms_hash,'lifecycle_snapshot_hash',p_lifecycle_snapshot_hash,'scope_snapshot_hash',p_scope_snapshot_hash)));
  v_receipt:=public.mhidas_ticket_reserve_operation_receipt_v9('channel_mutation','user',auth.uid(),'auth:user',null,null,'authorize_activate_commercial_channel_v2',p_idempotency_key,'commercial_channel',p_channel_id,p_expected_lock_version,v_request_hash,p_correlation_id,v_receipt_policy);
...
create function public.mhidas_resolve_public_event_ticket_action_v8(p_canonical_event_id uuid)
...
create policy commercial_partners_admin_read_v4116 on public.commercial_partners for select to authenticated using (public.mhidas_is_useclubbers_admin_v1(auth.uid()));
create policy event_ticket_channels_admin_read_v4116 on public.event_ticket_commercial_channels for select to authenticated using (public.mhidas_is_useclubbers_admin_v1(auth.uid()));
create policy event_ticket_requests_owner_or_admin_read_v4116 on public.event_ticket_partnership_requests for select to authenticated using (submitted_by_user_id=auth.uid() or public.mhidas_is_useclubbers_admin_v1(auth.uid()));
create policy event_ticket_signals_admin_read_v4116 on public.event_ticket_purchase_signals for select to authenticated using (public.mhidas_is_useclubbers_admin_v1(auth.uid()));
create policy event_ticket_audit_admin_read_v4116 on public.event_ticket_commercial_audit_log for select to authenticated using (public.mhidas_is_useclubbers_admin_v1(auth.uid()));

revoke all on table public.commercial_partners,public.commercial_partner_representatives,public.event_ticket_trusted_integrations,public.event_ticket_trusted_integration_credential_versions,public.event_ticket_trusted_integration_channels,public.event_ticket_partnership_requests,public.event_ticket_retention_policy_bootstrap_receipts,public.event_ticket_retention_policy_versions,public.event_ticket_retention_minimization_rules,public.event_ticket_url_validation_authorities,public.event_ticket_official_source_attestations,public.event_ticket_commercial_channels,public.event_ticket_operation_result_contracts,public.event_ticket_operation_receipts,public.event_ticket_verified_credential_contexts,public.event_ticket_click_attributions,public.event_ticket_purchase_signals,public.partner_official_communications,public.event_ticket_retention_runs,public.event_ticket_commercial_audit_log,public.event_ticket_backfill_rejections from PUBLIC,anon,authenticated,service_role;
...
revoke all on function public.mhidas_ticket_canonical_payload_v1(text,jsonb) from public;
...
revoke all on function public.mhidas_ticket_lock_current_credential_v3(uuid,uuid) from public;
revoke all on function public.mhidas_ticket_assert_current_credential_invariant_v1() from public;
revoke all on function public.mhidas_ticket_assert_retention_execution_contract_v1(uuid,text,integer) from public;
revoke all on function public.mhidas_ticket_reserve_operation_receipt_v9(text,text,uuid,text,uuid,uuid,text,text,text,uuid,integer,text,text,uuid) from public;
revoke all on function public.mhidas_ticket_complete_operation_receipt_v6(uuid,text,integer,text) from public;
revoke all on function public.mhidas_ticket_fail_operation_receipt_v6(uuid,text) from public;
revoke all on function public.mhidas_ticket_issue_verified_credential_context_v6(uuid,uuid,text,text,text,text,text,text,text,text,timestamptz,text) from public;
revoke all on function public.mhidas_ticket_consume_verified_credential_context_v7(uuid,text,text,text,text,uuid,uuid,uuid,uuid) from public;
revoke all on function public.mhidas_record_event_ticket_purchase_signal_v11(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb,uuid,text,text) from public;
revoke all on function public.mhidas_run_event_ticket_governance_retention_batch_v7(integer,text,text) from public;
...
revoke all on function public.mhidas_admin_retire_event_ticket_retention_policy_v7(uuid,uuid,integer,text,text,text) from public;
revoke all on function public.mhidas_admin_authorize_activate_commercial_channel_v2(uuid,integer,text,text,text,text,text) from public;
revoke all on function public.mhidas_ticket_url_proof_is_fresh_v8(uuid,text,text,text,timestamptz,timestamptz,timestamptz,text,integer,text,text,text,text,text,text,text,text,text,text,uuid) from public;
revoke all on function public.mhidas_ticket_official_source_proof_is_fresh_v1(uuid,text,text,text,text,text,text,text,text,text,text,integer,text,timestamptz,timestamptz,uuid) from public;
revoke all on function public.mhidas_resolve_public_event_ticket_action_v8(uuid) from public;
...
grant execute on function public.mhidas_ticket_issue_verified_credential_context_v6(uuid,uuid,text,text,text,text,text,text,text,text,timestamptz,text) to service_role;
grant execute on function public.mhidas_record_event_ticket_purchase_signal_v11(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb,uuid,text,text) to service_role;
grant execute on function public.mhidas_run_event_ticket_governance_retention_batch_v7(integer,text,text) to service_role;
...
grant execute on function public.mhidas_admin_authorize_activate_commercial_channel_v2(uuid,integer,text,text,text,text,text) to authenticated;
grant execute on function public.mhidas_resolve_public_event_ticket_action_v8(uuid) to anon,authenticated;

-- =============================================================================
-- 13. ELEVENTH-CORRECTED STRUCTURAL SELF-CHECK
-- =============================================================================

do $mhidas_v4116_self_check$
declare v_count integer;
begin
  select count(*) into v_count from public.event_ticket_retention_policy_versions where policy_key like '%_br_template';
  if v_count<>7 then raise exception 'V4_8_116_RETENTION_POLICY_TEMPLATE_COUNT_INVALID'; end if;
  select count(*) into v_count from public.event_ticket_retention_policy_versions where policy_key like '%_br_template' and policy_status='active' and bootstrap_receipt_id='00000000-0000-4116-8000-000000000000'::uuid;
  if v_count<>7 then raise exception 'V4_8_116_RETENTION_POLICY_BOOTSTRAP_ACTIVE_COUNT_INVALID'; end if;
  select count(*) into v_count from public.event_ticket_retention_minimization_rules where rule_status='active';
  if v_count<>44 then raise exception 'V4_8_116_MINIMIZATION_RULE_COUNT_INVALID'; end if;
  select count(*) into v_count from public.event_ticket_operation_result_contracts where contract_status='active';
  if v_count<>9 then raise exception 'V4_8_116_RESULT_CONTRACT_COUNT_INVALID'; end if;
  if (select provolatile from pg_proc where oid='public.mhidas_ticket_resolve_active_retention_policy_v2(text,text,text)'::regprocedure)<>'v' then raise exception 'V4_8_116_RETENTION_RESOLVER_NOT_VOLATILE'; end if;
  if not exists (select 1 from pg_trigger where tgname='event_ticket_integrations_current_credential_v4116_ct' and tgdeferrable) then raise exception 'V4_8_116_INTEGRATION_CONSTRAINT_TRIGGER_MISSING'; end if;
  if exists (select 1 from public.event_ticket_trusted_integrations i where i.integration_status='active' and (i.current_credential_version_id is null or i.issuer_service_key_hash is null)) then raise exception 'V4_8_116_ACTIVE_INTEGRATION_AUTHORITY_DRIFT'; end if;
  if exists (select integration_id from public.event_ticket_trusted_integration_credential_versions where credential_status='active' group by integration_id having count(*)>1) then raise exception 'V4_8_116_MULTIPLE_ACTIVE_CREDENTIALS'; end if;
  if exists (select 1 from public.event_ticket_commercial_channels c where c.channel_status='active' and (c.authorization_receipt_id is null or c.financial_terms_hash is null or c.lifecycle_snapshot_hash is null or c.authorized_scope_snapshot_hash is null or c.url_validation_authority_id is null or c.url_proof_signature is null or c.destination_url_ciphertext is null or c.valid_from is null)) then raise exception 'V4_8_116_ACTIVE_CHANNEL_AUTHORITY_DRIFT'; end if;
  if exists (select 1 from public.event_ticket_purchase_signals s where s.external_transaction_hash is not null and s.transaction_family_id is null) then raise exception 'V4_8_116_SIGNAL_FAMILY_ID_DRIFT'; end if;
  if exists (select 1 from public.event_ticket_operation_receipts r where r.receipt_status='completed' and not exists (select 1 from public.event_ticket_operation_result_contracts c where c.operation_name=r.operation_name and c.result_status=r.result_status and c.contract_status='active' and r.result_version>=c.minimum_result_version and (c.maximum_result_version is null or r.result_version<=c.maximum_result_version))) then raise exception 'V4_8_116_RECEIPT_RESULT_CONTRACT_DRIFT'; end if;
  if exists (
  ...
  ) then raise exception 'V4_8_116_RETIRED_POLICY_SUCCESSOR_DRIFT'; end if;
  if exists (
    select 1
    from unnest(array[
      'commercial_partners','commercial_partner_representatives','event_ticket_trusted_integrations',
  ...
  ) then raise exception 'V4_8_116_SERVICE_ROLE_EFFECTIVE_DML_GRANT_DRIFT'; end if;
  ...
  if v_count<>5 then raise exception 'V4_8_116_RLS_SELECT_GRANT_COUNT_INVALID'; end if;
  if position('destination_url_ciphertext' in pg_get_function_result('public.mhidas_resolve_public_event_ticket_action_v8(uuid)'::regprocedure))>0 then raise exception 'V4_8_116_PUBLIC_RESOLVER_EXPOSES_CIPHERTEXT'; end if;
  if exists (
    ...
    where n.nspname='public'
      ...
      and (p.proname like 'mhidas_ticket_%' or p.proname like 'mhidas_record_event_ticket_%' or p.proname like 'mhidas_admin_%' or p.proname like 'mhidas_resolve_public_event_ticket_%')
      and p.oid not in (
        'public.mhidas_ticket_issue_verified_credential_context_v6(uuid,uuid,text,text,text,text,text,text,text,text,timestamptz,text)'::regprocedure,
        'public.mhidas_record_event_ticket_purchase_signal_v11(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb,uuid,text,text)'::regprocedure,
        'public.mhidas_run_event_ticket_governance_retention_batch_v7(integer,text,text)'::regprocedure
      )
  ) then raise exception 'V4_8_116_SERVICE_ROLE_PRIVILEGED_ROUTINE_ALLOWLIST_DRIFT'; end if;
end;
$mhidas_v4116_self_check$;

-- =============================================================================
-- 14. EXPLICIT PROTECTED-DRAFT DECISION
-- =============================================================================

-- parsed_postgres_statement_count=127
-- draft_decision=eleventh_corrected_adjusted_draft_ready_for_twelfth_structural_review
-- normalized_schema_from_base=True
-- corrected_adjustments=10
-- critical_corrections=6
-- high_corrections=4
-- retention_lineage_columns_on_policy_table=True
-- active_retention_policy_bootstrap_receipt_first=True
-- nullable_admin_authorization_fail_closed=True
-- nullable_credential_proof_fail_closed=True
-- commercial_url_proof_fail_closed=True
-- active_channel_redeemable_and_time_bound=True
-- purchase_signal_correction_chain_open=True
-- retention_rules_execution_authority=True
-- expired_context_lifecycle_closed=True
-- legacy_privileged_bypass_inventory_closed=True
-- base_preflight_closed_index_inventory=True
-- credential_key_possession_verified=True
-- credential_context_request_payload_bound=True
-- commercial_url_proof_destination_signature_bound=True
-- official_fallback_authority_hostname_signature_bound=True
-- signal_retention_family_atomic=True
-- service_role_direct_table_dml_closed=True
-- retired_policy_lineage_and_drain_closed=True
-- signed_payload_canonicalization_session_independent=True
-- receipt_terminal_result_contract_closed=True
...
-- v4_8_114_tenth_corrected_sql_changed=False
